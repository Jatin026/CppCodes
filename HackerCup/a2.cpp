...
ifstream in("perfectly_balanced_chapter_2_input.txt");
    ...
    while (q--)
    {
        ...
        if(type==1){
            ...
            if((r-l+1)%2==1){
                --l;--r;
                int mid =(l+r)/2;
                if(m.count(((ft.sum(mid,r)-ft.sum(l,mid-1)+N)%N))) ans++;
                else{
                    if(m.count(((ft.sum(l,mid)-ft.sum(mid+1,r)+N)%N))) ans++;
                }
                // debug((((st.sum(l,(r+l)/2+1)-st.sum((l+r)/2,r+1))+N)%N))
                // debug((((st.sum((r+l)/2,r+1)-st.sum(l,(l+r)/2))+N)%N))
