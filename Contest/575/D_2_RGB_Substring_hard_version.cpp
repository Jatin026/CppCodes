...
void solve(){
    ...
    cin >> n >> k;
    string s; cin >> s;
    vector<vector<vi>> f (n+1 , vvi (3 , vi (3)));
    for (int i = 0; i < n; i++)
    {
        f[i+1] = f[i];
        if(s[i] == 'R') f[i+1][0][i%3]++;
        if(s[i] == 'G') f[i+1][1][i%3]++;
        if(s[i] == 'B') f[i+1][2][i%3]++;
    }
    int ans = 1e9;
    for (int i = 0; i + k - 1< n; i++)
    {
        int req = (k+2)/3 , curr = 0;
        curr -= f[i+k][0][i%3] - f[i][0][i%3];
        curr += req;
        req = (k+1)/3;
        curr -= f[i+k][1][((i%3) + 1)% 3] - f[i][1][((i%3) + 1)% 3];
        curr += req;
        req = k/3;
        curr -= f[i+k][2][((i%3) + 2)% 3] - f[i][2][((i%3) + 2)% 3];
        curr += req;
        ans = min(ans , curr);
        curr = 0;
        req = (k+2)/3 ;
        curr -= f[i+k][1][i%3] - f[i][1][i%3];
        curr += req;
        req = (k+1)/3;
        curr -= f[i+k][2][((i%3) + 1)% 3] - f[i][2][((i%3) + 1)% 3];
        curr += req;
        req = k/3;
        curr -= f[i+k][0][((i%3) + 2)% 3] - f[i][0][((i%3) + 2)% 3];
        curr += req;
        ans = min(ans , curr);
        curr = 0;
        req = (k+2)/3 ;
        curr -= f[i+k][2][i%3] - f[i][2][i%3];
        curr += req;
        req = (k+1)/3;
        curr -= f[i+k][0][((i%3) + 1)% 3] - f[i][0][((i%3) + 1)% 3];
        curr += req;
        req = k/3;
        curr -= f[i+k][1][((i%3) + 2)% 3] - f[i][1][((i%3) + 2)% 3];
        curr += req;
        ans = min(ans , curr);
    }
    cout << ans <<nline;
