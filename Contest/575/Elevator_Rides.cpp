...
void solve(){
    ...
    for(int mask = 0 ; mask < (1<<n) ; mask++){
        for(int i = 0 ; i < n ; i++){
            if(!((mask)&(1<<i))){
                if(dp[mask  +  (1 << i)].first > dp[mask].first + ((dp[mask].second + v[i])  >= x)){
                    dp[mask  +  (1 << i)].first = dp[mask].first + ((dp[mask].second + v[i])  >= x);
                    if(((dp[mask].second + v[i])  > x)) dp[mask  +  (1 << i)].second = min(dp[mask].second , v[i]);
                    else if(((dp[mask].second + v[i])  == x)) dp[mask  +  (1 << i)].second = 0;
                    else dp[mask  +  (1 << i)].second = dp[mask].second + v[i];
                }
                else if((dp[mask  +  (1 << i)].first == dp[mask].first + ((dp[mask].second + v[i])  >= x))){
                    if(((dp[mask].second + v[i])  > x)) dp[mask  +  (1 << i)].second = min(dp[mask  +  (1 << i)].second ,  min(dp[mask].second , v[i]));
                    else if(((dp[mask].second + v[i])  == x)) dp[mask  +  (1 << i)].second = 0;
                    else dp[mask  +  (1 << i)].second = min(dp[mask  +  (1 << i)].second , (dp[mask].second + v[i]));
                }
            }
        }
    }
