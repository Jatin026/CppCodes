...
/*
    ...
    Start and End time of Projects 
...
void solve(){
    ...
    for(int i = 0 ; i < n ;  i++){
        int a , b , w;
        cin >> a >> b >> w;
        edges.emplace_back(array<int,3> {a,b,w});
    }
    sort(all(edges) , [&](const array<int,3> &a , const array<int,3> &b ) {
    ...
    {
        debug(edges[i][0])   
        auto it = maxi.lb(edges[i][0]);
        --it;
        if(maxi.count(edges[i][1])) maxi[edges[i][1]] = max(maxi[edges[i][1]] , (it->second) +  edges[i][2]);
        else maxi[edges[i][1]] = (it->second) +  edges[i][2];
        ans = max(ans , (it->second) +  edges[i][2]);
        it = maxi.lb(edges[i][1]);
        --it;
        maxi[edges[i][1]] = max(maxi[edges[i][1]] , (it->second));
    }
    debug(maxi)
    cout << ans;
}
