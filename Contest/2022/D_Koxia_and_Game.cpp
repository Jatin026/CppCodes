...
void solve(){
    ...
    Mint ans = 1;
    dsu d(n+1);
    vi self_loop(n+1,0) , edges(n+1,0);
    for (int i = 0; i < n; i++)
    {
        if(!d.same(a[i],b[i])){
            int tot = edges[d.leader(a[i])] + edges[d.leader(b[i])];
            int x  = d.leader(a[i]) ,  y = d.leader(b[i]);
            d.merge(a[i],b[i]);
            self_loop[d.leader(a[i])] |= self_loop[y];
            self_loop[d.leader(b[i])] |= self_loop[x];
            edges[d.leader(a[i])] = tot+1;
        }
        else{
            if(a[i] == b[i]) {
                ans*= n;
                self_loop[d.leader(a[i])] = 1;
                edges[d.leader(a[i])]++;
            }
            else{
                if(edges[d.leader(a[i])] != 2*d.size(a[i])) ans = 0;
                else if(self_loop[d.leader(a[i])]) ans*= n;
                else ans*= 2;
            }
        }
    }
