...
void solve(){
    ...
    map<vector<bool>,ll> dp;
    dp[v] = 1;
    for (int i = 0; i < k; i++)
    {
        map<vector<bool> , ll> new_dp;
        for(auto x : dp){
            auto temp = x.first;
            for (int j = 0; j < n; j++)
            {
                if(j+1 < n && x.first[j] && !x.first[j+1]){
                    swap(temp[j],temp[j+1]);
                    new_dp[temp] += x.second;
                    if(new_dp[temp] >= mod) new_dp[temp] -= mod;
                    swap(temp[j],temp[j+1]);
                }
                if(j > 0 && x.first[j] && !x.first[j-1]){
                    swap(temp[j],temp[j-1]);
                    new_dp[temp] += x.second;
                    if(new_dp[temp] >= mod) new_dp[temp] -= mod;
                    swap(temp[j],temp[j-1]);
