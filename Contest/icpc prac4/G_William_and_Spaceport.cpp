...
void solve(){
    ...
    dfs(0,0);
    function<void(int,int)> pre = [&](int v , int p) -> void {
        contri[v] = 2*(sub[v]*(n-sub[v]+1)-1);
        for(auto u : adj[v]){
            if(u == p) continue;
            pre(u,v);
        }
    ...
    for (int i = 0; i < n; i++)
    {
        ms.insert(make_pair(contri[i],val[i]));
    }
    while(k > 0){
        if(ms.empty()) break;
        auto it = ms.rbegin();
        pair<ll,ll> v = (*it);
        ms.erase(ms.find((*it)));
        ll cut = min(k,v.second);
        k -= cut;
        v.second -= cut;
        if(v.second > 0) ms.insert(v);
    }   
    ll ans  = 0;  
    for(auto x: ms){
