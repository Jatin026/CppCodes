...
struct segtree{
    ...
    void init(int n){
        ...
        // prod.assign(2*size,1);
        max2.assign(2*size,v);
    }
    void build(vector<int> &a,int x , int lx , int rx){
        ...
        // prod[x]=prod[2*x+1]*prod[2*x+2];
         
        int ele2 = max(max2[2*x+1][1],max2[2*x+2][1]);
        if(max2[2*x+1][0]>max2[2*x+2][0]){
            max2[x][0]=max2[2*x+1][0];
            max2[x][1]=max(ele2,max2[2*x+2][0]);
        }
        else{
            max2[x][0]=max2[2*x+2][0];
            max2[x][1]=max(ele2,max2[2*x+1][0]);
        }
    }
    ...
    vi sum(int l , int r, int x , int lx , int rx){
        vi v(2,-1e9);
        if(l>=rx || lx>=r) return v;
        if(l<=lx && r>=rx) return max2[x];
         
            int m = (lx+rx)/2;
            vi s1=sum(l,r,2*x+1,lx,m);
            vi s2=sum(l,r,2*x+2,m,rx);
            int ele2 = max(s1[1],s2[1]);
            if(s1[0]>s2[0]){
                v[0]=s1[0];
                v[1]=max(ele2,s2[0]);
            }
