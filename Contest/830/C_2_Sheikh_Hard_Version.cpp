...
void solve(){
    ...
    while (q--)
    {
        ...
        ans = prefS[r]-prefS[l-1] - (prefX[r]^prefX[l-1]);
        ...
        for (int i = l; i <= r; i++)
        {
            if(ans != prefS[r]-prefS[i-1] - (prefX[r]^prefX[i-1])) break;
            if(v[i-1]==0) i = jump[i-1]+1;
            int la = i , ra = r , f = r , s = i;
            while (la<=ra)
            {
                    int mid = (la + ra)/2;
                    if(prefS[mid]-prefS[i-1] - (prefX[mid]^prefX[i-1]) == ans){
                        ra = mid - 1;
                        f = mid;
