...
void solve(){
    ...
    for (int i = 0; i < n; i++)
    {
        ll a, b;
        cin >> a >> b;
        ele.emplace_back(a,b);
        f.pb(a); s.pb(b);
    }
    sort(all(ele));
    sort(all(f));
    sort(all(s));
    ll ans = min(f[n-1] - f[n-2] , s[n-1] - s[n-2]);
    set<int> curr;
    vll suff(n+1,-1e9);
    for(int i = n-1 ; i > -1 ; i--){
        suff[i] = max(suff[i+1] , ele[i].second);
    }
    debug(ans)
    for (int i = 0; i < n; i++)
    {
        ans = min(ans , (ll)abs(ele[i].first - suff[i+1]));
        if(i+1 < n) ans = min(ans , (ll)abs(ele[i].first - max(ele[i+1].first , suff[i+2])));
        // debug(ans)
        if(i > 0) ans = min(ans , (ll)abs(ele[i].first - max(ele[i-1].first , suff[i+1])));
     
        if(!curr.empty()){
            auto it = curr.lb(ele[i].first);
            if(it != curr.end()){
                ans = min(ans , (ll)abs((ll)ele[i].first - max(suff[i+1] , (ll)(*it))));
            }
            if(it != curr.begin()){
                --it;
                ans = min(ans , (ll)abs((ll)ele[i].first - max(suff[i+1] , (ll)(*it))));
        ...
        debug(ans)
        curr.insert(ele[i].second);
    }
    curr.clear();
    for(auto &x : ele) swap(x.first,x.second);
    sort(all(ele));
    for(int i = n-1 ; i > -1 ; i--){
        suff[i] = max(suff[i+1] , ele[i].second);
    }
