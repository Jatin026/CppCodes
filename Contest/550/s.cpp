...
void solve(){
    ...
    vi par(n,-1) , depth(n);
    vector<array<int,2>> pre(n,array<int,2> {0,0});
    ...
    const int K = 17;
    vvi bl(n,vi(K,-1));
    for(int i = 0 ; i < n ; i++) bl[i][0] = par[i];
    for(int j = 0 ;  j < n; j++){
        for(int i = 1 ; i < K; i++){
    ...
    auto lca = [&](int u , int v) -> int {
        ...
        if(depth[u] != depth[v]) jump(u,depth[v] - depth[u]);
        if(u == v) return v;
        for(int i = K - 1 ; i > -1 ; i--){
            if(bl[v][i] != bl[u][i]) {
                v = bl[v][i];
    ...
    while(q--){
        ...
        cnt[l&1]++;
        debug(cnt[0])
        odd += (x*1ll*cnt[1]); eve += (x*1ll*cnt[0]);
    }
    cout << abs(odd - eve);
}
int main(){
