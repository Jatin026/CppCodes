...
void solve(){
    ...
    for(int i = 0 ; i < n; i++){
        ...
        if(a[i] < ans){
            ...
            tot.emplace_back(c[i] , a[i] - max(ans , b[i] - 1));
        }
    }
    long long cost = 0;
    sort(begin(tot) , end(tot));
    for(auto x : tot){
        cost += min(req , (long long)x.second)*(long long)(x.first);
        req -= min(req , (long long)x.second);
    }
    cout << ans << " " << cost;
}   
int main(){
