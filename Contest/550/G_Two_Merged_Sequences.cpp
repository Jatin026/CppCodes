...
void solve(){
    ...
    {
        int cnt = 0;
        if(inc.empty() || inc.back() < v[i]) cnt++;
        if(dec.empty() ||  dec.back() > v[i]) cnt++;
        if(cnt == 0){    
            cout << "NO\n";
            return;
        }
        else if(cnt == 1){
            if(inc.empty() || (!inc.empty() && inc.back() < v[i])) inc.pb(v[i]) , ans[i] = 0;
            else if(dec.empty() || (!dec.empty() && dec.back() > v[i])) dec.pb(v[i]);
            else{
                cout << "NO\n";
            }
        }
            ...
            if(i+1 < n){
                if(v[i+1] == v[i]) inc.pb(v[i]) , ans[i] = 0;
                else if(v[i+1] > v[i]){
                    inc.pb(v[i]); ans[i] = 0;
                }
                else dec.pb(v[i]);
            }
            else inc.pb(v[i]) ,  ans[i] = 0;
        }
           debug(inc) debug(dec)
    }
    cout << "YES\n";
