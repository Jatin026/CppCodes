...
// using the fact that the sum that will be given as output can be constructed by subtracting the largest given values each time 
// this will give the visited cell
ll M = mod2 ;
map<ll,bool> dp;
int n;
map<pr,ll> weight;
...
void solve(){
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < n; j++)
        {
            weight[make_pair(i,j)] = (hash(i*j+i*53+j*31))%M;  
            cout << weight[make_pair(i,j)] << " ";
