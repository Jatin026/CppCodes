...
bool check(ll height , ll days, vi &v){
    ll odd = 0  , req  = 0 , even = 0;
    for (int i = 0; i < v.size(); i++)
    {
        if((height-v[i])%2==1){    
            odd++;
        }
        else if((height-v[i])%2==0){
            even ++;
        }
        req+=((height-v[i])/3)*2;
    }
...
void solve(){
    ...
    debug(check(maxHeight,16,v));
    ll ans = -1 , l = 0 , r = 1e12;
    while(l<=r){
        ll mid = (l+r)/2;
        if(check(maxHeight,mid,v)|check(maxHeight+1,mid,v)){
            ans = mid;
            r = mid-1;
