...
void build(string a, int v, int tl, int tr, int i) {
    if (tl == tr && tl < a.size()) {
        t[i][v] = (a[tl] - 'a' == i);
    } else {
        int tm = (tl + tr) / 2;
...
void update(int v, int tl, int tr, int pos, char new_val, int i) {
    if (tl == tr) {
        t[i][v] = (new_val - 'a' == i);
    } else {
        int tm = (tl + tr) / 2;
...
void solve(){
    ...
    loop(i,0,q,1){
        ...
        if(typ == 1){
            ...
            char x;
            cin >> pos >> x;
            update(1, 0, n - 1, pos - 1, x, a[pos-1]-'a');
            update(1, 0, n - 1, pos - 1, x, x - 'a');
            a[pos - 1] = x;
        }
        else{
            int l, r;
