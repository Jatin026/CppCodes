...
void solve(){
    ...
    for (int i = 1; i < m; i++)
    {
        int maxi = max(v[i],v[i-1]) , mini = min(v[i],v[i-1]);
        if(maxi == mini) continue;
      
        if(1 <= (mini-1)) {
            ans[mini] -= (maxi-mini); ans[1] += (maxi-mini);
        }
        if((maxi-1)>=mini) {ans[mini] += (maxi-1); ans[mini+1] -= (maxi-1);}
         
        if((mini+1) <= (maxi-1)){ ans[maxi] -= (maxi-1-mini); ans[mini+1] += (maxi-1-mini);}
        if(maxi >= (mini + 1)){ ans[maxi+1] -= mini ; ans[maxi] += mini;}
        if((maxi + 1) <= n) {ans[n+1] -= (maxi-mini); ans[maxi+1] += (maxi-mini);}
