...
void solve(){
  ...
  rep(i, 1, n + 1){
    rep(j, 1, m + 1){
      make_set((i - 1) * m + (j - 1) + 1);
    }
  }
  
  vector<vbl> down(n+1, vbl(m+1, false));
  vector<vbl> right(n+1, vbl(m+1, false));
  rep(i, 0, e){
    ll x1, y1, x2, y2;
    cin >> x1 >> y1 >> x2 >> y2;
    if(x1 == x2){
      down[x1 - 1][min(y1, y2)] = true;
    }
    else{
      right[min(x1, x2)][y1 - 1] = true;
    }
  }
  ...
  rep(i, 1, n * m + 1){
    if(!vis[find_set(i)]){
      vis[find_set(i)] = true;
      moves.push_back(sizes[find_set(i)]);
    }
  }
  sort(rbegin(moves),rend(moves));
