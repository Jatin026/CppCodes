...
void solve(){
    ...
    for (int i = 0; i < n; i++)
    {
        if(range.empty()) range.emplace_back(s[i],1);
        else{
            if(range.back().first == s[i]) range.back().second++;
            else range.emplace_back(s[i],1);
        }
    }
    int len = range.size();
    for (int i = 0; i < len; i++)
    {
        int w = range[i].second;
        for (int bal = 0; bal <= n; bal++)
        {
            if(bal + w <= n) dp[i+1][bal+w] |= dp[i][bal];
            if(bal - w >= 0) dp[i+1][bal-w] |= dp[i][bal];
        }
    }
    debug(dp)
    if(dp[len][0]){
        ...
        int curr = 0 ,ind = len;
        while(ind){
            if(curr + range[ind-1].second <= n && dp[ind-1][curr+range[ind-1].second]){
                while(range[ind-1].second--){
                    ans += ')';
                    curr++;
                }
            }
            else{
                if(curr >= range[ind-1].second  && dp[ind-1][curr-range[ind-1].second]){
                    while(range[ind-1].second--){
                        ans +='(';
                        curr--;
                    }
