...
void solve(){
    ...
    int fin = 0;
    for(int i = 0; i < n ; i++){
        int l = i , r = i + suff[i] - 1, mid , ans = i-1;
        while(l <= r){
            mid = (l+r)/2;
            int j = __lg(mid - i + 1);
            int gc = gcd(st[j][i], st[j][mid - (1 << j) + 1]);
            if(gc > 1){
                l = mid + 1;
                ans = mid;
            }
        ...
        ans += suf1[ans+1];
        debug(ans)
        auto it = ub(all(pref) , k + pref[i]);
        --it;
        r = min((int)(it - begin(pref)) - 1,ans);
        debug(r)
        if(pref[r+1] - pref[i] <= k) fin = max(fin , r + 1 - i);
        debug(fin)
    }
    cout << fin << nline;
}
