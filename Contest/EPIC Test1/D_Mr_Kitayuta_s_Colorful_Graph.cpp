...
void solve(){
    ...
    vvi presence(m);
    vvi which(n);
    vector<unordered_map<int,vi>> col(m);
    for (int i = 0; i < m; i++)
    {
        int a , b , c;
        cin >> a >> b >> c;
        ...
        presence[c].pb(a);
        presence[c].pb(b);
        which[a].pb(c); which[b].pb(c);
    }
    for (int i = 0; i < n; i++)
    {
        sort(all(which[i]));
        which[i].resize(unique(all(which[i])) - which[i].begin());
    }
    for (int i = 0; i < m; i++)
    {
                sort(all(presence[i]));
        presence[i].resize(unique(all(presence[i])) - presence[i].begin());
    }
    
    vector<unordered_map<int,int>> vis(m);
    for (int i = 0; i < m; i++)
    {
        int cnt = 1;
    ...
    for (int i = 0; i < q; i++)
    {
        ...
        cin >> a >> b;
        --a; --b;
        querry.emplace_back(a,b);
        qq[a].emplace_back(b,i);
        qq[b].emplace_back(a,i);
    }
    debug(vis)
    vi heavy(n);
    for (int i = 0; i < n; i++)
    {
        if(in_degree[i] >= B){
            ...
            for(auto x : which[i]){
                for(auto y : vis[x]){
                    if(y.second == vis[x][i]) cnt[y.first]++;
                }
            }
            for(auto x : qq[i]){
    ...
    for(int i = 0 ; i < querry.size() ; i++){
        if(heavy[querry[i].first] || heavy[querry[i].second]) continue;
        debug(querry[i].first)
        debug(which[querry[i].first])
        for(auto u : which[querry[i].first]){
            if(vis[u].count(querry[i].second) && vis[u][querry[i].first] == vis[u][querry[i].second]) ans[i]++;
        }
    }
