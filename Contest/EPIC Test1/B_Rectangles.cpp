...
void solve(){
    ...
    pr la, ra;
    la.first = max(l[0].first , l[1].first);
    la.second = max(l[0].second , l[1].second);
    ra.first = min(r[0].first , r[1].first);
    ra.second = min(r[0].second , r[1].second);
    if(ra.first <= la.first){
        cout << 0;
        return;
    }
    if(ra.second <= la.second){
        cout << 0;
        return;
    }
    la.first = max(la.first , l[2].first);
    la.second = max(la.second , l[2].second);
    ra.first = min(ra.first , r[2].first);
    ra.second = min(ra.second , r[2].second);
    if(ra.first <= la.first){
        cout << 0;
        return;
    }
    ...
    cout << (1ll)*(ra.first - la.first)*(ra.second - la.second);
}
int main(){
    FAST
    int t=1;
