...
void solve(){
    ...
    for(int i = 1 ; i*i <= x ; i++){
        if(x%i == 0){
            ...
            int r = 0;
            for(auto x : q){
                while(r < p.size() && p[r].first < x.first) r++;
                if(r < p.size() && p[r].first == x.first) {
                    p[r].second -= x.second;
                }
            }
            ll tot_div = 1;
            ...
            if(tot_div < n) continue;
            vector<Mint> ans(n+1);
            debug(tot_div)
            for (int pos = 1; pos <= n; pos++)
            {
                ans[pos] = pre[tot_div][pos];
                for (int j = 1; j < pos; j++)
                {
                    ans[pos] -= dp[j][n-j]*ans[j];
                }
            }
        ...
        if(x%i == 0 && (x-x/i >= x/i)){
            ...
            for(auto x : q){
                while(r < p.size() && p[r].first < x.first) r++;
                if(r < p.size() && p[r].first == x.first) p[r].second -= x.second;
            }
            ll tot_div = 1;
            for(auto x : p){
                tot_div *= (x.second+1);
