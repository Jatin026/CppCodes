...
const lld eps = 1e-6;
void solve(){
    ll aa , ax0 , ay0 , az0 , ab , ax1 ,ay1 , az1;
    cin >> aa >> ax0 >> ay0 >> az0 >>  ab >> ax1 >> ay1 >> az1;
    lll a = aa , x0 = ax0, y0 = ay0 , z0 = az0 , b = ab , x1 = ax1 , y1 = ay1 , z1 = az1;
    
    ll cnt = 0;
    if(a != b){
        for (ll x = -1000  ; x < 1000 ; x++)
        {
            lll A = b - a  , B = 2*a*y0 - 2*b*y1 , C = b*y1*y1 - a*y0*y0 + z1 - z0 + b*(x-x1)*(x-x1) - a*(x-x0)*(x-x0);
            lll D = B*B - 4*A*C;
            if(D < 0) continue;
            if(D == 0) cnt++;
            else{
