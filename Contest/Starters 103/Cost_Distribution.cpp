...
void dfs(int v, int p = -1) {
    ...
    for (int to : adj[v]) {
        ...
        if (visited[to]) {
            ...
            if (low[to] > tin[v]){
                cost[make_pair(to , v)] = cost[make_pair(v, to)] = ((1LL) * wt[{v,to}] * (n - sub[to]) * sub[to]);
            }
        }
    }
}
...
void solve(){
    ...
    find_bridges();
    vi degree(n);
    vector<vector<pair<ll,ll>>> new_adj(n);
    for(auto x : cost){
        new_adj[x.first.first].emplace_back(x.first.second , x.second);
        // new_adj[x.first.second].emplace_back(x.first.first , x.second);
        degree[x.first.second]++;
        // degree[x.first.first]++;
    }

    auto check = [&](ll available) -> bool {
        queue<int> q;
        auto temp = degree;
        vi vis(n , 0);
        for(int  i = 0; i < n; i++){
        ...
        while(!q.empty()){
            ...
            for(auto &u : new_adj[v]){
                ...
                ll weight = u.second;
                weight -= min(weight , rem[v]);
                rem[u.first] -= weight;
                temp[u.first]--;
                // temp2[{u.first , v}] =temp2[{v , u.first}] =  0; 
        ...
        // for(auto x : temp2) if(x.second != 0) return 0;
        for(auto x : rem) if(x < 0) return 0;
        return 1;
    };
