...
void solve(){
    ...
    if(n <= 2){
        ...
        for(int i = 1; i < BinExpItr(10 , n) ; i++){
            string k = to_string(i);
            int cnt = 0;
            if(i%8 == 0 && k.size() == s.size()){
                for(int l = 0; l < n; l++){
                    if(k[l] != s[l]){
                        cnt++;
                    }
        ...
        cout << ans.second << nline;
    }
    else{
        if(stoi(s.substr(n - 3, 3))%8 == 0) cout << s << nline;
        else{
            for(int i = 0; i < 10 ; i++){
                s[n-1] = i + '0';
                if(stoi(s.substr(n - 3, 3))%8 == 0) {cout << s << nline ;return;}
            }
