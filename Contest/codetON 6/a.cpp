...
void solve(){
    ...
    vpr ans;
    for(int i = 0; i < n; i++){
        if(ans.empty() || ans.back().first < v[i]) ans.emplace_back(v[i],i);
        else{
            while(!ans.empty() && ans.back().first >= v[i]) ans.pop_back();
            ans.emplace_back(v[i],i);
        }
    }
    int k;
    cin >> k;
    vi fin(n+1);
    fin[0] += (k/ans[0].first);
    fin[ans[0].second+1] -= (k/ans[0].first);
    int dif = ans[0].first , prv = ans[0].second + 1 , pp = k / ans[0].first;
    k %= ans[0].first;
    for(int i = 1 ; i < ans.size(); i++){
        int l = i ,  r = (int)ans.size() - 1, x = i , mid;
        while(l <= r){
            mid = (l + r)/2;
            if(min(pp,k/(ans[i].first - ans[i-1].first)) == min(pp,(k / (ans[mid].first - ans[i-1].first)))){
                l = mid + 1;
                x = mid;
            }
            else r = mid - 1;
        }
        int sum = k/(ans[x].first - ans[i-1].first);
        pp = min(pp , sum);
        sum = min(sum , pp);
        fin[ans[x].second + 1] -= sum;
        fin[prv]  += sum;
        prv = ans[x].second + 1;
        k = k - sum*(ans[x].first - ans[i-1].first);
        i = x;
    }
