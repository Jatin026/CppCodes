...
void solve(){
    ...
    dp[0][0][0] = dp[0][0][1] = dp[1][0][0] = dp[0][1][1] = 1;
    string a , b ; cin >> a >> b;
    ...
    for (int i = 0; i <= n; i++)
    {
        for (int j = 0; j <= m; j++)
        {
            if(i == 0){
                if(j>0 && j < m){
                    if(b[j]!=b[j-1]){
                        dp[i][j+1][1] = 1 + dp[i][j][1];
                    }
                    else dp[i][j+1][1] = dp[i][j][1];
                } 
            }
            if(j == 0){
                if(i>0 && i < n){
                    if(a[i]!=a[i-1]){
                        dp[i+1][j][0] = 1 + dp[i][j][0];
                    }
                    else dp[i+1][j][0] = dp[i][j][0];
                } 
            }
            if(i<n && i>0){
                if(a[i]!=a[i-1]) dp[i+1][j][0] = min(dp[i+1][j][0],1 + dp[i][j][0]);
                else dp[i+1][j][0] = min(dp[i+1][j][0],dp[i][j][0]);
            }
            if(i<n && j>0){
                if(a[i]!=b[j-1]) dp[i+1][j][0] = min(dp[i+1][j][0],1 + dp[i][j][1]);
                else dp[i+1][j][0] = min(dp[i+1][j][0],dp[i][j][1]);
            }
            if(j<m && i>0){
                if(b[j]!=a[i-1]) dp[i][j+1][1] =  min(dp[i][j+1][1],1 + dp[i][j][0]);
                else dp[i][j+1][1] =  min(dp[i][j+1][1],dp[i][j][0]);
            }
            if(j<m && j>0){
                if(b[j]!=b[j-1]) dp[i][j+1][1] = min(dp[i][j+1][1],1 + dp[i][j][1]);
                else dp[i][j+1][1] = min(dp[i][j+1][1],dp[i][j][1]);
            }
            
        }
    }
    cout << min(dp[n][m][0],dp[n][m][1]) <<nline;
}
int main(){
    FAST
    int t=1;
