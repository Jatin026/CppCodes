...
void solve(){
    int n; cin>>n;
    vi a(n),b(n); for(auto &x : a) cin>>x; for(auto &x : b) cin>>x;
    vi fire,frost;
    vpr v;
    for (int i = 0; i < n; i++)
    {
    ...
    sort(all(fire));sort(all(frost));
    vpr a1,a2;
    int l=frost.size()-1;
    for (int r = fire.size()-1; r > -1; r--)
    {
        a1.pb({fire[r],1});
        if(l>-1){
            a1.pb({frost[l],0});
    ...
    debug(a1) debug(a2)
    reverse(all(a2));
    ll ans1=a1[0].first,ans2=a2[0].first;
    for (int i = 1; i < n; i++)
    {
        if(a1[i].second!=a1[i-1].second) ans1+=2*a1[i].first;
        else ans1+=a1[i].first;
        if(a2[i].second!=a2[i-1].second) ans2+=2*a2[i].first;
        else ans2+=a2[i].first;
    }
