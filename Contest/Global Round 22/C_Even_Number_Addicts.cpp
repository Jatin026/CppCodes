...
int dp[N][N][2][2];
bool ans(int even ,int odd , int p, int n){
    if(even<0 || odd <0) return false;
    if(even+odd==1){
        
        if(odd==1 ){
            if(p==odd && p==n) return 1;
            else return 0;
        }
        else{
            if(p==0  && p==n) return 1;
    ...
    if(dp[even][odd][x][n]!=-1) return dp[even][odd][x][n];
    bool flag=false;
    if(even>0){
        flag=(flag | !ans(even-1,odd,!(n^p),n));
    }
    if(odd>0){
        flag=(flag | !ans(even,odd-1,!(n^p),!n));
    }
    return dp[even][odd][x][n]=flag;
}
void solve(){
    ...
    if(ans(even,odd,0,(odd&1))) cout<<"Alice\n";
    else cout<<"Bob\n";
}
int main(){
    FAST
