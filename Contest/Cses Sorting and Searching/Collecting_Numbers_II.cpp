...
void solve(){
    ...
    while(q--){
        ...
        if(abs(a-b) == 1){
            ...
            if(a && ind[a] > ind[a-1]) ans++;
            if(a && ind[b] > ind[a-1]) ans--;
            if(a + 1 < n && ind[a + 1] > ind[a]) ans++;
            if(a + 1 < n && ind[a] > ind[b]) ans--;
            if(b + 1 < n && ind[b + 1] > ind[b]) ans++;
            if(b + 1 < n && ind[b + 1] > ind[a]) ans--;
            if(flag) swap(a,b);
        }
        else{
            if(a && ind[a] > ind[a-1]) ans++;
            if(a && ind[b] > ind[a-1]) ans--;
            if(a + 1 < n && ind[a + 1] > ind[a]) ans++;
            if(a + 1 < n && ind[a + 1] > ind[b]) ans--;
            if(b && ind[b] > ind[b-1]) ans++;
            if(b && ind[a] > ind[b-1]) ans--;
            if(b + 1 < n && ind[b + 1] > ind[b]) ans++;
            if(b + 1 < n && ind[b + 1] > ind[a]) ans--;
        }
        swap(ind[a] , ind[b]);
