...
/*
    Diverse -> cnt(distinct) >= cnt(max(freq of a char))
    maximum check has to be done only for string of size len .
...
//         // cnt of maximum frequency of an ele.
//     }
...
void solve(){
    ...
    string s; cin >> s;
    ll ans = 0;
    // vector<vector<string>> pre(n,vector<string> (100,""));
    for (int i = 0; i < n; i++)
    {
        string k = "";
        int start = i;
