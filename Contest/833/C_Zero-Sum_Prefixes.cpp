...
/*
significance of a[i] == 0 in prefix sum , the consecutive elements repeat;
score == cnt of 0 in prefix sum
...
void solve(){
    ...
    // {
    //     cnt[pref[i]]++;
    //     ans=max(ans,cnt[pref[i]]);
    // }
    // debug(cnt)
    // debug(pref)
    // cout << ans - (cnt0 != n) << nline;
    // for (int i = 1; i <= n; i++)
    // {
    //     if(!v[i-1]){
    //         v[i-1]=-(pref[i]-pref[ind]);
    //         ind = i;
    //     }
    // }
