...
void solve(){
    ...
    for (int i = 0; i < fac1.size(); i++)
    {
        ...
        {
            factor.pb(fac1[i]*fac2[j]);
        }
        
    }
    sort(all(factor));
    factor.resize(unique(all(factor))-factor.begin());
    debug(factor)
    int ans = 0,tot = 0;
    ...
    for (int i = 0; i < factor.size(); i++)
    {
        ...
        for(auto y : factor){
            if(y%x == 0){
                ...
                if(!flag) new_factor.pb(y);
            } 
            else new_factor.pb(y); 
        }
        cnt%=2;
        if(cnt) ans^=x;
        swap(new_factor,factor);
    }
