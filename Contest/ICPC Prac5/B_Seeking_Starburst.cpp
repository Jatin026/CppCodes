...
void solve(){
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < m; j++)
        {
            if(val[i][j] == 'S') s = make_pair(i,j);
            if(val[i][j] == 'E') f = make_pair(i,j);
        } 
    }
    ...
    for(char c  : {'B','W','C'}){
        ...
        while(!q.empty()){
            ...
            for (int dx = -1; dx < 2; dx++)
            {
                ...
                {
                    if(abs(dx)+abs(dy) == 1 && valid(v.ff+dx,v.ss+dy) && !vis[v.ff+dx][v.ss+dy] && (val[dx+v.ff][dy+v.ss] == c || (val[dx+v.ff][dy+v.ss] == 'E' && v!=s) ) ){
                        dis[v.ff+dx][v.ss+dy] = 1+dis[v.ff][v.ss];
                        vis[v.ff+dx][v.ss+dy] = 1;
                        q.push(make_pair(v.ff+dx,v.ss+dy));
                    }
                }
        ...
        if(dis[f.ff][f.ss] == inf){
            continue;
        }
        else{
            if(c == 'B'){
                pr prev = ans;
                ans = min(ans , make_pair(dis[f.ff][f.ss]*3,dis[f.ff][f.ss]*2));
                if(prev != ans) opt = 'B';
            }

            else if( c == 'W'){
                pr prev = ans;
                ans = min(ans , make_pair(dis[f.ff][f.ss]*10,dis[f.ff][f.ss]*5));
                if(prev != ans) opt = 'W';
            }
            else{
                pr prev = ans;
                ans = min(ans , make_pair(dis[f.ff][f.ss]*10,dis[f.ff][f.ss]*10));
                if(prev != ans) opt = 'C';
            }
        }
    }
