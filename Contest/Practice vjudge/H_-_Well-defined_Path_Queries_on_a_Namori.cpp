...
void solve(){
    ...
    for (int i = 0; i < n; i++)
    {
        if(d.same(edges[i].first,edges[i].second) && !flag){
            st = {edges[i].first,edges[i].second};
            flag = 1;
        }   
        else {
           d.merge(edges[i].first,edges[i].second);
            temp[edges[i].first].pb(edges[i].second);
            temp[edges[i].second].pb(edges[i].first);
        }
    }
    vi par(n+1,-1),vis(n+1,0);
    queue<int> q;
    q.push(st.first);
    vis[st.first] = 1;
    ...
    for (int i = st.second; i != -1 ; i=par[i])
    {
        cursed[i] = 1;
    }
    ...
    dsu ans(n+1); 
    for(auto x : edges){
        if(cursed.count(x.first) + cursed.count(x.second) !=2 ){
            ans.merge(x.first,x.second);
        }
    }
    int qq;
    cin >> qq;
