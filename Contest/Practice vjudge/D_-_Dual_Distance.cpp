...
void pre(int v  , vvi &adj , vi &vis , vll &size , vll &dist){
    vis[v] = 1;
    for(auto u : adj[v]){
        if(vis[u]) continue;
        dist[u] = 1 + dist[v];
        pre(u,adj,vis,size,dist);
        size[v]+=size[u];
    }
}
void solve(){
    int n , k;
    cin >> n >> k;
    vvi adj(n+1);vi vis(n+1,0);vll size(n+1,1) , ans(n+1,0) , dist(n+1,0);
    for (int i = 0; i < n-1; i++)
    {
        int a , b; cin >> a >> b;
        adj[a].pb(b);
