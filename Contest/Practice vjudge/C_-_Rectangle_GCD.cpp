...
struct segtree{
    ...
    ll ans(int l , int r, int x , int lx , int rx){
        if(l>=rx || lx>=r) return 0;
        if(l<=lx && r>=rx) return sums[x];
         
        int m = (lx+rx)/2;
        ll s1=ans(l,r,2*x+1,lx,m);
        ll s2=ans(l,r,2*x+2,m,rx);
        return gcd(s1,s2);
        
    }
    ll ans(int l , int r){
        return ans(l,r,0,0,size);
    }
};
...
void solve(){
    int n , q;
    cin >> n >> q;
    vi a(n) , b(n) , difa , difb;
    for(auto &x : a){
        cin >> x;
    }
    difa.pb(a[0]);difb.pb(b[0]);
    for(auto &x : b){
        cin >> x;
    }
    for (int i = 1; i < n; i++)
    {
        difa.pb(abs(a[i]-a[i-1]));difb.pb(abs(b[i]-b[i-1]));      
    }
    segtree A , B;
    A.init(n);B.init(n);
    A.build(difa);B.build(difb);
    debug(difb)
    for (int i = 0; i < q; i++)
    {
        int h1 , h2, w1 , w2;
        ...
        if(h1>h2) swap(h1,h2);
        if(w1>w2) swap(w1,w2);
        ll ans = a[h1-1]+b[w1-1];
        if(h1 == h2){
            if(w1 == w2){
                 
            }
            else{
                ans = gcd(ans,B.ans(w1,w2));
            }
        }
        else{
            if(w1 != w2){
                ans = gcd(ans,gcd(B.ans(w1,w2),A.ans(h1,h2))) ;
            }
            else ans =gcd(ans, A.ans(h1,h2));
        }
        cout << ans << nline;
    }
