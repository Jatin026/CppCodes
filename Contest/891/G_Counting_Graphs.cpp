...
struct dsu{
    ...
    void merge_set(int u , int v , int maxi , int s){
        make_set(v);
        make_set(u);
        u = leader(u); v = leader(v);
        if(u == v) return;
        if(size[u] > size[v]) swap(u,v);
        ans[v] = (ans[v]*ans[u])%mod2;
        if(s >= maxi)
        ans[v] *= (BinExpItr(s-maxi + 1, (ll)size[u]*size[v] - 1) );
        ans[v] %= mod2;
        par[u] = v;
        size[v] += size[u];
    }
...
void solve(){
    ...
    for (int i = 0; i < n - 1; i++)
    {
        ...
        cin >> a >> b >> w;
        --a;--b;
        edges.emplace_back(array<int,3> {w,a,b});
    }
    dsu d(n);
    sort(all(edges));
    for(auto x : edges){
        d.merge_set(x[1],x[2],x[0],s);
    }
    cout << d.ans[d.leader(0)] << nline;
}
int main(){
    FAST
    int t=1;
