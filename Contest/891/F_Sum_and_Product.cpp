    ...
    for (int i = 0; i < q; i++)
    {
        ...
        if((x*x - 4*y) < 0){
            ...
            if((((x%2) + 2 ) % 2) == 1) cout << 0 << " ";
                ...
                if(f.count(x/2)) cout << f[x/2]*(f[x/2] - 1)/2  << " ";
                else cout << 0  << " ";
            }
        }
        else{
            ll val = sqrtll(x*x  - 4*y);
            if(val*val != (x*x  - 4*y)){
                cout << 0 << " ";
                continue;
            }
            else{
                if(((2+((x+val)%2))%2) == 1) cout << 0 << " ";
                else{
                    if(f.count((x-val)/2) && f.count((x+val)/2)) cout << f[(x-val)/2]*f[(x+val)/2] << " ";
                    else cout << 0 << " ";
                } 
            }
        }
