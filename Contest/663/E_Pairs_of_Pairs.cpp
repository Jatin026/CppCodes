...
void dfs(int vertex , vector <vi> &v , vi &vis , vi &subc, vi &par){ 
    vis[vertex]=1;
    for(auto child : v[vertex]){
        if(vis[child]) continue;
        subc[child]=subc[vertex]+1;   
        par[child]=vertex;
        dfs(child,v,vis,subc,par);
...
void solve(){
    ...
    h=0;
    vi par(n+1,-1),col(n+1,0);
    fill(all(subc),0);
    dfs(node,v,vis,subc,par);
    int dest=-1;
    for (int i = 0; i < n+1; i++)
    {
        if(h<subc[i]){
    ...
    if(h>=(n+1)/2){
        ...
        for (int i = 0; i < n; i++)
        {
            for (int j = 0; j < depth[i].size() ; j+=2)
            {
                if(j+1<depth[i].size()) ans.pb({depth[i][j],depth[i][j+1]});
            }
