...
void solve(){
    ...
    {
        vll new_dp(k,1e18);
        vll new_value(k,1e18);
        ll val = (1ll<<i);
       
        if((1ll<<i)&n){
            for (ll rem = 0; rem < k; rem++)
            {
                if(dp[rem] <= new_dp[rem]){
                    new_value[rem] =  value[rem] ;
                }
                new_dp[rem] = min(dp[rem],new_dp[rem]);
                 if(dp[rem] + 1 <= new_dp[(rem - (val%k)+k)%k]){
                    new_value[(rem - (val%k)+k)%k] = (value[rem]&((~(val))));
                }
                new_dp[(rem - (val%k)+k)%k] = min(dp[rem]+1,new_dp[(rem - (val%k)+k)%k]);
            }
            ...
            for (ll rem = 0; rem < k; rem++)
            {
                if(dp[rem] + 1 <= new_dp[(rem+val)%k]){
                    new_value[(rem+val)%k] = (value[rem]|(val));
                }
                new_dp[(rem+val)%k] = min(dp[rem]+1,new_dp[(rem+val)%k]);
                if(dp[rem] <= new_dp[rem]){
                    new_value[rem] =  value[rem] ;
                }
                new_dp[rem] = min(dp[rem],new_dp[rem]);
