...
void solve(){
    ...
    q.push(end[0]);
    vis[end[0].ff][end[0].ss] = 1;
    dist[end[0].ff][end[0].ss] = 0;
    while(!q.empty()){
        pr source  = q.front();
        q.pop();
        for(auto u : dir){
            if(isVALID(source.ff + u.ff , source.ss + u.ss)){
                if(vis[source.ff + u.ff][source.ss + u.ss]) continue;
                if(v[source.ff + u.ff][source.ss + u.ss] != '#'){
                    dist[source.ff + u.ff][source.ss + u.ss] = 1 + dist[source.ff][source.ss];
                    vis[source.ff + u.ff][source.ss + u.ss] = 1;
                    if(v[source.ff + u.ff][source.ss + u.ss] == '*') break;
                    q.push(make_pair(source.ff + u.ff , source.ss + u.ss));
    ...
    for(auto x : start ) {
        ...
        int shortest = 1e9;
        if(x == start[0]){
        ans1 += dist[end[0].ff][end[0].ss];
        ans2 += dist[end[1].ff][end[1].ss];
        }
        else{
            ans1=max(ans1,dist[end[1].ff][end[1].ss]);
            ans2=max(ans2,dist[end[0].ff][end[0].ss]);
        }
        for(auto x : end){
            shortest = min(shortest , dist[x.ff][x.ss]);
        }   
        ans = min(ans , shortest + small );
    }
    ans = min(ans , ans1);
    ans = min(ans ,ans2);
    if(ans >= 1e9){
