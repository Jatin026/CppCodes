...
void solve(){
    int n;
    cin >> n;
    vector<vector<pair<int,int>>> pref(n) , suff(n);
    vi v(n);
    for(auto &x : v) cin >> x;
    ...
    for(int i = 0; i < n; i++){
        ...
        for(auto x : curr){
            int val = gcd(x.first , v[i]);
            if(where.count(val)) where[val] = max(where[val] , x.second);
            else where[val] = x.second;
        }
        if(where.count(v[i])) where[v[i]] = max(where[v[i]] , i);
        else where[v[i]] = i;
        for(auto x : where) new_curr.emplace_back(x);
        swap(new_curr,curr);
        sort(all(curr));
        pref[i] = curr;
       debug(curr)
    ...
    for(int i = n - 1; i > -1; i--){
        ...
        for(auto x : curr){
            int val = gcd(x.first , v[i]);
            if(where.count(val)) where[val] = min(where[val] , x.second);
            else where[val] = x.second;
        }
        if(where.count(v[i])) where[v[i]] = min(where[v[i]] , i);
        else where[v[i]] = i;
        for(auto x : where) new_curr.emplace_back(x);
        swap(new_curr,curr);
        sort(all(curr));
    ...
    for(int i = 0; i < n; i++){
        ...
        for(int j = 0 ; j < pref[i].size() ; j++){
            for(int k = 0 ; k < suff[i].size(); k++){
                if(gcd(pref[i][j].first , suff[i][k].first) == 1){
                    ll first, second;
                    if(j){
                        first = abs(pref[i][j].second - pref[i][j-1].second);
                    }
                    else{
                        first = pref[i][j].second + 1;
                    }
                    if(k){
                        second = abs(suff[i][k].second - suff[i][k-1].second);
                    }
                    else{
                        second = n - suff[i][k].second;
                    }
                    ans += first*second;
