...
const int N = 1e6 + 7;
vi inda(N,-1),indb(N,-1);
bool check(ll days , vll &sol , ll k ,ll n , ll m , ll g ){
    ll lcm  = (n*m)/g;
    ll dif = 0;
    for (ll i = 0; i < n; i++)
    {
        ...
        if(sol[i] == -1){
            ...
            ll same = 0 , d = (days-i-1)/n + 1;
            if(days>=sol[i]){
                same = (days-sol[i])/lcm + 1;
            }
            dif+= d - same;
        }
          debug(dif)
...
void add(ll &s , ll lcm){
    if(s > -1 && s < lcm + 1) return;
...
void solve(){
    ...
    for (int i = 0; i < n; i++)
    {
        if(inda[a[i]] != -1 && indb[a[i]] != -1  && abs(inda[a[i]]-indb[a[i]])%g == 0){
            sol[i] = inda[a[i]] + (x*(indb[a[i]] - inda[a[i]])*(n/g));
            add(sol[i],(n*m)/g); 
        }   
    }
    ll ans = 1 , l = 1 , r = 1e18 , mid;
    while(l<=r){
        mid = (l+r)/2;
        if(check(mid,sol,k,n,m,g)){
            r = mid - 1;
            ans = mid;
        }
        else l = mid + 1;
