...
void solve(){
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        cin >> ele;
        v[i] = ele;
        if(ind[ele].size()<2) ind[ele].pb(i+1);
        if(ind[ele].size()>1){
            s.insert(ele);
        }
    }
    if(s.size()>1){
        ...
        cout << "YES\n";
        debug(v)
        cout << ind[a][0] <<" "<<ind[b][0] <<" "<<ind[a][1] <<" "<<ind[b][1] << nline;
        return;
    }
    else if(s.size() == 1){
        int a = *(s.begin());
        ...
        while(l<r){
            ...
            if(r+l>a){
                ...
                if(l == r || ind[l][0] == ind[a/2][0] || ind[l][0] == ind[a/2][1] ||  ind[r][0] == ind[a/2][0] || ind[r][0] == ind[a/2][1]) continue;
                cout << "YES\n";
                debug(a)
                cout << ind[a/2][0] << " "<<ind[a/2][1]<<" "<<ind[l][0]<<" "<<ind[r][0]<<nline;
                return;
            }
        }
    }
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        for (int j = n-1; j > i; j--)
        {
            ...
            if( sum[v[i]+v[j]] == make_pair(-1,-1)){
                sum[v[i]+v[j]]=make_pair(ind[v[i]][0],ind[v[j]][0]);
          
            }
            else{
                cout << "YES\n";
                cout << ind[v[i]][0] <<" "<< ind[v[j]][0] <<" "<<sum[v[i]+v[j]].first <<" "<<sum[v[i]+v[j]].second<<nline;
                return;
            }
            if(cnt > 1000 ) break;
        }
