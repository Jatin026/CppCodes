...
void solve(){
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < n; j++)
        {
            ans.insert(min(v[0],gcd(v[i],v[j])));
        }
    }
    ...
    for(auto x : fac){
        ...
        for (int i = 0; i < n; i++)
        {
            if(v[i]%x == 0) gg=gcd(gg,v[i]);
        }
        if(x == gg) ans.insert(min(x,v[0]));
        debug(gg)
    }
    // ans.insert(v[0]);
    // bool flag = true;
    ...
    //         {
    //             int val = gcd(v[i],x);
    //             if(ans.find(val) == ans.end()) inc.pb(val); 
    //         }
    //     }
    //     inc.resize(unique(all(inc))-inc.begin());
    //     for(auto x : inc){
    //         ans.insert(x);
    //     }
    //     if(inc.size() < 1 || cnt>5) flag = 0;
    //     else a = inc;
    //     cnt++;
    // }
    ans.insert(g);
