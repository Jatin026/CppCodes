...
void solve(){
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        for (int j = 0; j < m; j++)
        {
            while(!q.empty() && q.back().second > v[i][j]){
                q.pop_back();
            }
            q.push_back({j+1,v[i][j]});
            while(!q.empty() && j+1 - q.front().first >= b){
                q.pop_front();
            }
            if(j+1>=b) temp.pb(q.front().second);
        }
        ans.pb(temp);
    }
    debug(v)
    ...
    for (int j = 0; j < m- b + 1; j++)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            while(!q.empty() && q.back().second > ans[i][j]){
                q.pop_back();
            }
            q.push_back({i+1,ans[i][j]});
            while(!q.empty() && i+1 - q.front().first>=a ){
                q.pop_front();
            }
            if(i+1>=a) sum+=q.front().second;
        }
