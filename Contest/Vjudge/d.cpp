...
struct segtree{
    ...
    void init(int n, int m){
        ...
        maxi.assign(2*sizex,vi(2*sizey,0));
    }
    void build_y(vvi &a , int x , int y , int ly , int ry  ,  int rx , int lx){
        if(ry-ly==1){
            if(rx-lx==1 && lx<lenx && ly<leny) maxi[x][y]=a[lx][ly];
            if(rx-lx!=1){ 
                maxi[x][y]=(std::max(maxi[2*x+1][y],maxi[2*x+2][y]));
            }
            return;
        }
        int mid = (ly+ry)/2;
        ...
        maxi[x][y]=(std::max(maxi[x][2*y+1],maxi[x][2*y+2]));
    ...
    void set_y(int xx , int yy , int v ,int x , int y , int ly , int ry , int rx , int lx){
        if(ry-ly==1){
            if(rx-lx == 1) maxi[x][y]=v;
            else maxi[x][y]=std::max(maxi[2*x+1][y],maxi[2*x+2][y]);
            return;
        }
        int m = (ly + ry)/2;
        if(m>yy){
    ...
    ll max_y(int x , int y1 , int y2 , int y , int ly , int ry){
        if(y1>=ry || ly>=y2) return (-INF);
        if(y1<=ly && y2>=ry) return maxi[x][y];
        int m = (ly+ry)/2;
        ll s1 = max_y(x , y1 ,y2,2*y+1,ly,m);
        ll s2 = max_y(x,  y1, y2,2*y+2,m,ry);
        return (std::max(s1,s2));
    }
    ll max_x(int x1 , int y1 , int x2 , int y2, int x , int lx , int rx){
        if(x1>=rx || lx>=x2) return (-INF);
        if(x1<=lx && x2>=rx){
            return max_y(x,y1,y2,0,0,sizey);
        }
        int m = (lx+rx)/2;
        ll s1 = max_x(x1,y1,x2,y2,2*x+1,lx,m);
        ll s2 = max_x(x1,y1,x2,y2,2*x+2,m,rx);
         
        
        return (std::max(s1,s2));
        
    }
    ll max(int x1 , int y1 , int x2 , int y2){
        return max_x(x1,y1,x2,y2,0,0,sizex);
    }
};
...
void solve(){
    ...
    ll mini = INF;
    int row = 0, col = 0;
    while(row != n - k +1 || col != m-k +1 ){
        mini = min(mini , st.max(row,col,row+k,col+k));
        col++;
        if(col == m-k + 1){
            col = 0;
            row++;
