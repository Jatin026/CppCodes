/*
*every vertex has a range
*find maximum beauty 
beauty  ???  summation | au - av | .
* maximizing total sum of weight of the edges of the tree
where weight is defined as the absolute diff. between | au - av |.
*give the value at the root of the tree as the minimum/maximum.
// dp solution.
* can create a 2D DP , defining such that what is the sum till that node if we took mini or maximum of the range.
*Assuming its beneficial to choose only these two.
dp[u][1] = max(dp[v][0] + abs(lv - ru) , dp[v][1] + abs(rv -  ru) )
// greedy solution 
look at every vertex what maximum value we can choose wrt child.
...
void dfs(int v , vi &left , vi &right, vll &val){
    vis[v]=1;
    for(auto u : adj[v]){
        if(vis[u]) continue;
        dfs(u,left,right,val);
        if(val[v] == 1) break;
        dp[v][1] +=   max(dp[u][1] + abs(right[u]-right[v]) , dp[u][0] + abs(left[u]-right[v]))  ;
        dp[v][0] +=  max(  dp[u][1] + abs(right[u]-left[v]) ,  dp[u][0] + abs(left[u]-left[v])) ;
