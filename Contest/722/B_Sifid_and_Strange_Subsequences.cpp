/*
 |ai - aj | >= MAX 
 * can i pick more than 1 positive number ?  NO , this is the reason why for all positive element ans is 1.
        check whether is there any number that i can pick , using the difference array ?
 * its good to pick all the non-positive number in the sequence.
 * 
...
void solve(){
    ...
    {
        mini = min(mini , abs(neg[i]-neg[i+1]) );
    }
    if(neg.size() == 1){
        cout << 2 << nline; 
        return;
