...
void solve(){
    ...
    for(auto &x : v){
        cin >> x;
        highest = max(highest,__lg(x)+1);
    }
    
    ll ans = (n*(n+1))/2;
    xorArr[0] = v[0];
    for (int i = 1; i < n; i++)
        xorArr[i] = xorArr[i - 1] ^ v[i];
   
    for(auto x : ind){
        long long k = 0; // Initialize answer to be returned
        vi mp((5*n),0);
        bool flag = true;
        for (int i = 0; i < n; i++) {
...
int main(){
    ...
    {
        if(isPerfectSquare(i+1)) ind.pb(i+1);
    }
    debug((int)ind.size())
    while(t--){
        solve();
