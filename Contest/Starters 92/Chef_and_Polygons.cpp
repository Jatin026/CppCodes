...
void solve(){
    ...
    cin >> n;
    vll x(n) , y(n);
    for(auto &xx : x) cin >> xx; for(auto &xx : y) cin >> xx;
    vll val;
    ll mini = 1e18, maxi = -1;
    for (int i = 0; i < n; i++)
    {
        ll g = gcd(abs(y[(i+1)%n]-y[i]),abs(x[(i+1)%n] - x[i]));
        ll yy = (y[(i+1)%n]-y[i])/g , xx = (x[(i+1)%n]-x[i])/g;
        for (int j = 0; j < g; j++)
        {
            val.push_back(yy*j + y[i] + x[i] + j*(xx));
            mini = min(mini , val.back()); maxi = max(maxi , val.back());
        }
    }
     
    ll ans = 0;
    auto r = segmentedSieve(mini,maxi);
