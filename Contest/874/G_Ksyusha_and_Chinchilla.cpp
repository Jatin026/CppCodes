...
void solve(){
    ...
    for(int i = 0 ; i < min(n,100) ; i++){
        ...
        function<void(int,int)> anss = [&](int v , int p) {
            vvi cnt(3);
            if(!flag) return;
            for(auto u : adj[v]){
                if(u == par[v]) continue;
            ...
            if(cnt[2].size() > 1 || cnt[1].size() > 2 || (cnt[1].size()&cnt[2].size()) ){
                flag = false;
                return;
            }
            for(auto x : cnt[0]){
        ...
        anss(i,i);
        if(!flag || n%3 != 0 || ans.size() < n/3 - 1) continue;
            cout << ans.size() << nline;
