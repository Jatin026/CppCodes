...
void solve(){
    ...
    for(auto x : v){
        if(f[x] == 1){
            if(x > mex) once.insert(x);
            f.erase(x);
        }
    }
    for(auto x : v){
        if(x < mex && f.count(x)) f.erase(x);
    }
    int steps = 0;
    int ans = distinct - mex;
    while(k && (!once.empty() || !f.empty()) && mex < n + 1 ){
        ans = min(ans , distinct  - mex);
        steps++;
        if(steps >= 2*n) break;
        debug(mex)
        if(!once.empty()){
                auto val = (*once.rbegin());
                once.erase(once.find(val));
                mex++;
                k--;
        }
        else{
        ...
        while(1){
            if(!once.empty() && once.count(mex)){
                once.erase(mex);
                mex++;
            }
            else{
                if(!f.empty() && f.count(mex)){
                    f.erase(mex);
                    mex++;
                }
                else {
                    if(once.empty() && !f.empty() && k){
                        auto it = f.lb(mex);
                        debug(f)
                        debug(mex)
                        debug(k)
