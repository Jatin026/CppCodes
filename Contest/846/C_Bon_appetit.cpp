...
void solve(){
    ...
    multiset<int> demand,table;
    for(auto &x : c){
        cin >> x;
        table.insert(x);
    }
    for(auto x : freq){
        demand.insert(x.second);
    }
    int satisfied = 0;
    while(demand.size()> 0 && table.size()>0){
        int customer  = (*demand.rbegin());
        demand.erase(demand.find(customer));
        auto it  = table.lb(customer);
        if(it == table.end()) --it;
        int available = (*it);
        satisfied += min(customer,available);
        customer -= min(customer,available);
        if(customer > 0){
            demand.insert(customer);
        }
        table.erase(it);
    }
    cout << satisfied << nline;
}
int main(){
    FAST
    int t=1;
