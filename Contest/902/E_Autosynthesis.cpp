...
void solve(){
    ...
    while(!q.empty()){
        ...
        in_degree[child[tp]]--;
        col[child[tp]] = max(col[child[tp]] , col[tp] ^ 1);
        if(!in_degree[child[tp]]){
            q.push(child[tp]);
        }
    }
    for(int i =0 ; i<n; i++){
        if(in_degree[i] == 1){
            q.push(i);
            start.pb(i);
            int len = 0 , prev = col[i];
            while(!q.empty()){
                int tp = q.front(); q.pop();
                in_degree[child[tp]]--;
                col[child[tp]] = max(col[child[tp]] , col[tp] ^ 1);
                if(in_degree[child[tp]] == 0 && child[tp] != i){
                    q.push(child[tp]);
                }
            }
        }
