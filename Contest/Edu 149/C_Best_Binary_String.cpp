...
void solve(){
    ...
    string c(n,'?');
    if(c == s){
        cout << string(n,'1') << nline;
        return;
    }
    vector<pair<char,int>> comp;
    for (int i = 0; i < n; i++)
    {
        if(comp.empty()) comp.emplace_back(s[i],1);
        else{
            if(comp.back().first == s[i]) comp.back().second++;
            else comp.emplace_back(s[i],1);
        }
    }
    string ans = "";
    for (int i = 0; i < comp.size(); i++)
    {
        if(comp[i].first == '1' || comp[i].first == '0'){
            while(comp[i].second--){
                ans += comp[i].first;
            }
        }
            ...
            if(comp.size() > i + 1){
                while(comp[i].second--) ans += comp[i+1].first;
            }
            else{
                while(comp[i].second--) ans += comp[i-1].first;
            }
