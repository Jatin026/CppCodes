...
const int N = 2005;
vector<vector<lld>> dp(N,vector<lld> (N));
void solve(){
    // dp[0][0] = 1;
    int n,t;
    lld p;
    cin >> n >> p >> t;
    for (int peeps = 0; peeps <= n; peeps++)
    {
        for (int time = 0; time <= t; time++)
        {  
            dp[peeps][time+1] += (1-p)*(dp[peeps][time]);
            dp[peeps+1][time+1] += (p)*(dp[peeps][time]+1);
        }
        
    }
    
    cout << fixed << setprecision(20) << dp[n][t] ;
}
int main(){
    FAST
    int t=1;
