...
void solve(){
    int n;
    cin >> n;
    vvi mini(n + 2, vi(2)) , maxi(n + 2, vi(2));
    maxi[0] = vi (2,1);
    int curr = 0;
    for (int i = 0; i < n; i++)
    {
        ...
        if(type == '+'){
            ...
            --v;
            curr++;
            maxi[curr][1] = x + maxi[v][1];
            maxi[curr][0] = max(maxi[v][0] , maxi[curr][1]);
            if(maxi[curr][1] < 0) maxi[curr][1] = 0;
            mini[curr][1] = x + mini[v][1];
            mini[curr][0] = min(mini[v][0] , mini[curr][1]);
            if(mini[curr][1] > 0) mini[curr][1] = 0;
        }
        else{
            int u , v , k;
            cin >> u >> v >> k;
            --v;
            if(k >= mini[v][0] && k <= maxi[v][0]) cout << "YES\n";
            else cout << "NO\n";
        }
    }
