...
void solve(){
    ...
    for (int i = 0; i < n-1; i++)
    {
        if(s[i] == s[i+1]){
            if(s[i] == '('){
                firs.insert(mk(i,i+1));
            }
            else last.insert(mk(-i,-(i+1)));
        }
    }
    vi val(n);
    for (int i = 0; i < n; i++)
    ...
    for (int i = 0; i < q; i++)
    {
        ...
        if(val[ind] == 1){
            ...
            if(ind > 0){
                if(val[ind-1] == 1){
                    firs.erase(mk(ind-1,ind));
                }
                else{
                    last.insert(mk(-(ind-1),-(ind)));
                }
            }
            if(ind < n-1){
                if(val[ind+1] == 1){
                    firs.erase(mk(ind,ind+1));
                }
                else{
                    last.insert(mk(-ind,-(ind+1)));
                }
            }
        }
        else{
            ...
            if(ind > 0){
                if(val[ind-1] == 1){
                    firs.insert(mk(ind-1,ind));
                }
                else{
                    last.erase(mk(-(ind-1),-(ind)));
                }
            }
            if(ind < n-1){
                if(val[ind+1] == 1){
                    firs.insert(mk(ind,ind+1));
                }
                else{
                    last.erase(mk(-ind,-(ind+1)));
                }
            }
        }
         
        int l = -1 ,r = n;
        if(!firs.empty()) l = firs.begin()->first;
        if(!last.empty()) r = -(last.begin()->second);
        // debug(f.first)
        bool flag = true;
        if(l == -1 && r == n && val[0] == 1 && ft.sum(0,n-1) == 0){
            cout << "YES\n";
            continue;
        }
        if(ft.sum(0,n-1) %2 != 0) flag = false;
        if(l == -1 || r == n){
            cout << "NO\n";
            continue;
        }
        if(!firs.empty() && l>0 && ft.sum(0,l-1) != 0){
            flag = false;
        }
        // debug(flag)
        if(!last.empty() && r < n - 1 && ft.sum(r+1,n-1) != 0){
            flag = false;
        }
        ...
        // debug(l)
        // debug(r)
        if(flag &&  (ft.sum(0,r)%2) == 0 && (ft.sum(l,n-1)%2) == 0) cout << "YES\n";
        else cout << "NO\n";
    }
...
/*
    ...
    dp[i][j] = b ko ith index tak process karo and j size ka subsequence bana lo a ka.
    (k-1) choices to make it same 
        dp[i][j+1] = dp[i][j]
        dp[i+1][j] = (k-1)*dp[i][j]
    dp(n*m)
    ->   dp[m][n]
    ...
    b should have a subsequence of a , b has a prefix of a from [1,n].
    total ways of constructing b - not good
    b : [1,i] 
    mCi : fixing i indexes such that I will put elements [1,i] from a on these indexes.
    a1 : 
    i indexes are fixed, of prefix in m.
    we are constructing from the first possible subeqeunce prefix in b.
    target: ----*----*-----
