...
void solve(){
    ...
    {
        if(range.empty()){
            range.emplace_back(a[i],1);
        }
        else{
            if(range.back().first == a[i]) range.back().second++;
            else range.emplace_back(a[i],1);
        }
    }
    for(auto x: range){
        aa[x.first] = max(aa[x.first],x.second);
    }
    range.clear();
    for (int i = 0; i < n; i++)
    {
        if(range.empty()){
            range.emplace_back(b[i],1);
        }
        else{
            if(range.back().first == b[i]) range.back().second++;
            else range.emplace_back(b[i],1);
        }
    }
    for(auto x: range){
        bb[x.first] = max(bb[x.first],x.second);
    }
    int ans = -1;
    for(auto x : aa){
        ans = max(ans , x.second + bb[x.first]);
    }
    for(auto x : bb){
        ans = max(ans , x.second + aa[x.first]);
    }
    cout << ans <<nline;
}
int main(){
