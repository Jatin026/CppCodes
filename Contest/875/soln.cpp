...
void solve(){
    int n;
    cin >> n;
    vector<unordered_map<int,int,custom_hash>> a(2*n+10);
    vi aa(n) , bb(n);
    for(auto &x : aa) cin >> x;
    for(auto &x : bb) cin >> x;
    for (int i = 0; i < n; i++)
    ...
    for (int i = 1; i <= sqrt(2*n); i++)
    {
        for (int j = i; j <= (2*n)/i; j++)
        {
            ...
            if(i != j){
                for(auto x : a[i]){
                    if(a[j].count(i*j-x.first))
                        ans += 1ll*x.second*a[j][i*j-x.first];
                }
            }
            else{
                ll sum =0;
                for(auto x : a[i]){
                    if(i*j == 2*x.first) sum += (1ll*x.second*(x.second-1)); 
                    else {
                        if(a[j].count(i*j-x.first))
                        sum += 1ll*x.second*a[j][i*j-x.first];
                        // ans += 1ll*x.second*a[j][i*j-x.first];
                    }
                }
                ans += sum/2;
