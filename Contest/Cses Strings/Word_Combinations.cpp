...
void add_string(const string &s){
    int curr = 0;
    for(auto x : s){
        if(trie[curr].next[x - 'a'] == -1){
            trie[curr].next[x - 'a'] = trie.size();
            trie.emplace_back();
        }
        curr = trie[curr].next[x - 'a'];
    }
    trie[curr].leaf++;
}
const int N = 5005;
array<array<int,N> , N> frq;
void solve(){
    string s;
    cin >> s;
    int k , n = s.size();
    ...
    for(int i = 0; i < n; i++){
        ...
        for(int j = i; j < n; j++){
            if(curr != -1){
                curr = trie[curr].next[s[j] - 'a'];
                if(curr != -1) frq[i+1][j + 1] = trie[curr].leaf;
            }
        }
    }
    vector<ll> dp(n + 1);
