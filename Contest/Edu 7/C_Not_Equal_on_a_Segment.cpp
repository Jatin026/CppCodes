...
struct node_max{
    ...
    node_max(int idx , int val) : idx(idx) , val(val){}
};
node_max max(node_max a, node_max b){
...
struct node_min{
    ...
    node_min(int idx , int val) : idx(idx) , val(val){}
};
node_min min(node_min a, node_min b){
    if(a.idx == b.idx) return a;
    if(a.val < b.val){
        return a;
    }
...
void solve(){
    ...
    for(auto &x : v) cin >> x;
    for(int  i = 0; i < n ; i++){
        maxi[0][i] = node_max(i + 1, v[i]);
        mini[0][i] = node_min(i + 1, v[i]);
    }
    for(int j = 1; j <= K; j++){
    ...
    auto ans = [&](int l , int r){
        int lg = __lg(r - l);
        return make_pair(max(maxi[lg][l] , maxi[lg][r - (1<<lg)]) , min(mini[lg][l] , mini[lg][r - (1<<lg)]));
    };
    while(m--){
        int l , r , x;
        cin >> l >> r >> x;
        --l;
        auto val = ans(l,r);
        if(val.first.val == x && val.second.val == x) cout << -1 << nline;
        else{
            assert(val.first.idx != -1);
            assert(val.second.idx != -1);
            if(val.first.val != x) cout << val.first.idx << nline;
            else cout << val.second.idx << nline;
        }
    }
}
int main(){
