...
void solve(){
    ...
    int ans = inf , len = ind.size();
    for (int i = 0; i < len; i++)
    {
        
        int l = i+1 , r = len - 1, mid , val = inf;
        while(l <=  r){
            mid = (l+r)/2;
            if(i == 1){
            debug()
        }
            if(pref[ind[mid]][0] - pref[ind[i]-1][0] > pref[ind[mid]][1] - pref[ind[i]-1][1] && pref[ind[mid]][0] - pref[ind[i]-1][0] > pref[ind[mid]][2] - pref[ind[i]-1][2]){
                r = mid - 1;
                val = ind[mid];
            }
            else l = mid + 1;
        ...
        if(val == inf) continue;
        ans = min(ans , val - ind[i] + 1);
    }
    debug(ind)
    if(ans == inf || ans < 2) cout << -1 << nline;
    else cout << ans << nline;
}
int main(){
    FAST
