...
void solve(){
    ...
    debug(left)
    debug(right)
    while(!ele[last].empty() && ele[last].size() <= left.size()){
        ans[left.back()] = ele[last].back();
        vis[ele[last].back()-1] = 1;
        ele[last].pop_back(); left.pop_back();
    }
    int l = left.size();
    for (int bit = 0; bit < 20; bit++)
    {
        if((1<<bit)&l){
            while(!ele[bit].empty()){
                ans[left.back()] = ele[bit].back();
                vis[ele[bit].back()-1] = 1;
                ele[bit].pop_back(); left.pop_back();
            }
        }
    }
