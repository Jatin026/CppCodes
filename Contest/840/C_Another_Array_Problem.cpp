...
void solve(){
    ...
    if(n == 2){
        cout << max(v[0]+v[1],2*abs(v[0]-v[1])) << nline;
    }
    else if(n>=4) cout << n*maxi << nline;
    else {
        sum = max(sum , n*v[n-1]);
        sum = max(sum, n*v[0]);
        sum = max(sum , 2*abs(v[0]-v[1]) + v[n-1]);
        sum = max(sum , v[0] + 2*abs(v[1]-v[2]));
        ll a  = abs(v[0] - v[1]);
        sum = max(sum,3*a);
        sum = max(sum , a + 2*abs(v[n-1]-a));
        sum = max(sum , 3*abs(v[n-1]-a));
        a = abs(v[1]-v[2]);
        sum = max(sum,3*a);
        sum = max(sum , 2*abs(v[0]-a) + v[n-1]);
        sum = max(sum , 3*abs(v[n-1]-a));
        cout << sum << nline;
    }
}
