...
void solve(){
    ...
    vector<array<ll,2>> dp(p+1,{(ll)1e9,-1});
    dp[0] = {0,0};
    for (int i = 1; i <= p; i++)
    {
        for(auto x : v){
           if(i-x.first>=0) {
                if(dp[i][0]>dp[i-x.first][0]+x.second){
                    dp[i][0]  = dp[i-x.first][0] + x.second;
                    dp[i][1] = dp[i-x.first][1] + x.second*dp[i-x.first][0];
                }
                else if(dp[i][0] == dp[i-x.first][0]+x.second){
                    dp[i][1] = max(dp[i][1] , dp[i-x.first][1] + dp[i-x.first][0]*x.second);
                }
           }
           else break;
        }
    ...
    // vi nodes;
    // while(temp>0){
    //     if(v[ind].first>temp) --ind;
    //     else{
    //         nodes.pb(v[ind].second);
    //         temp-=v[ind].first;
    //     }
    // }
    // for (int i = 0; i < 10; i++)
    ...
    // int n = nodes.size();
    // int tot = 0;
    // for(auto x : nodes) tot+=x;
    // for (int i = 0; i < n; i++)
    // {
    //     for (int j = i+1; j < n; j++)
    //     {
    //         ans+=nodes[i]*1ll*nodes[j];
    //     }     
    // }
    // cout << tot << " " << ans;
    cout << dp[p][0] <<" "<<dp[p][1];
