...
void solve(){
    ...
    {
        ele.clear(); cnt.clear(); depth.clear();
        ele.resize(n);   cnt.resize(n);depth.resize(n);
        for(auto u : adj[root]){
            depth[u] = 1;
            dfs(u,root);
            for (int i = 0; i < n; i++)
            ...
            for (int i = 1; i < n; i++)
            {
                if(cnt[i]) ele[i].pb(cnt[i]);
            }
            fill(all(cnt),0);fill(all(depth),0);
        }
        debug(ele)
        for(auto x : ele){
            if(x.size() >= k){
