...
void solve(){
    ...
    if(n&1 && !(m&1)){
        ...
        {
            if(mp){
                ans[n-1][j] = ans[n-1][j+1] = 'z';
            }
            else{
                ans[n-1][j] = ans[n-1][j+1] = 'y';
            }
            mp^=1;
        }
        k -= (m/2);
        ...
        {
            if(mp){
                ans[j][m-1] = ans[j+1][m-1] = 'x';
            }
            else{
                ans[j][m-1] = ans[j+1][m-1] = 'w';
            }
            mp^=1;
        }
        v -= (n/2);
    ...
    if(k >= 0 && !(k&1) && v>=0 && !(v&1)){
        ...
        for (int i = 0; i < n; i++)
        {
            for (int j = 0; j < m; j+=2)
            {
                if(k && ans[i][j] == '@' && i+1 < n && j+1 < m){
                    if((j/2)&1) ans[i][j] = ans[i][j+1] = 'a';
                    else ans[i][j] = ans[i][j+1] = 'c';
                    if((j/2)&1) ans[i+1][j] = ans[i+1][j+1] = 'b';
                    else ans[i+1][j] = ans[i+1][j+1] = 'd';
                    k-=2;
                }
            }
        }
        for (int i = 0; i < n; i++)
        {
            for (int j = 0; j < m; j+=2)
            {
                if(v && ans[i][j] == '@' && i+1 < n && j+1 < m){
                    if((i/2)&1) ans[i][j] = ans[i+1][j] = 'e';
                    else ans[i][j] = ans[i+1][j] = 'f';
                    if((i/2)&1) ans[i+1][j+1] = ans[i][j+1] = 'g';
                    else ans[i][j+1] = ans[i+1][j+1] = 'h';
                    v-=2;
                }
            }
        }
