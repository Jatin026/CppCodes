...
void solve(){
    ...
    debug((ll)init)
    ll prevValue = (*(cnt.begin())).first , prevCount=(*(cnt.begin())).second;
    debug(cnt)
    
        for(auto x :cnt){
            if(x.first ==  prevValue ) continue;
            if((x.first-prevValue)*prevCount > k){
              
                ll reach = k/prevCount;
                debug(reach)
                ans+=(sum-prevValue)*reach*prevCount  + (prevCount*(prevCount-1))*(reach)/2 + prevCount*(prevCount-1)*(reach*(reach-1))/2 ;
                sum+=reach*prevCount;
                debug((ll)ans)
                prevValue+=reach;
                k%=prevCount;
                ans+=k*(k-1)/2 + (sum-prevValue)*k;
                k=0;
                break;
            }
            else{
                ans+=(x.first-prevValue)*prevCount*(sum-prevValue) + (x.first-prevValue)*(prevCount*(prevCount-1))/2 + prevCount*(prevCount-1)*((x.first-prevValue)*(x.first-prevValue-1))/2  ;
                k-=(x.first-prevValue)*prevCount;
                sum+=(x.first-prevValue)*(prevCount);
                prevCount+=x.second;
                prevValue = x.first;
    ...
    if(k!=0){
        ll reach = k/prevCount;
        ans+= (sum-prevValue)*reach*prevCount   + (prevCount*(prevCount-1)*(reach-1))*(reach)/2 +  (prevCount*(prevCount-1))*(reach)/2 ; 
        debug(k)
