...
int find_set(int v, vll &size , vi &parent) {
    if (v == parent[v])
        return v;
    return parent[v] = find_set(parent[v],size,parent);
}
 
void make_set(int v, vll &size, vi &parent) {
    parent[v] = v;
    size[v] = 1;
}
 
void union_sets(int a, int b , vll &size, vi &parent) {
    a = find_set(a,size,parent);
    b = find_set(b,size,parent);
    if (a != b) {
        if (size[a] < size[b])
            swap(a, b);
        parent[b] = a;
...
void solve(){
    ...
    for (int i = 0; i < t; i++)
    {
        make_set(i+1,size1,parent1);
        make_set(i+1,size2,parent2);
    ...
    for (int i = 0; i < n; i++)
    {
        int a , b ; cin >> a >> b;
        rail[i]={a,b};
        union_sets(a,b,size1,parent1);
    }
    for (int i = 0; i < m; i++)
    {
        int a , b ; cin >> a >> b;
        road[i]={a,b};
        union_sets(a,b,size2,parent2);
        
    }
    map<pr,int> intersection;
    for (int i = 1 ; i <= t; i++)
    {
        int u = find_set(i,size1,parent1);
        int v = find_set(i,size2,parent2);
        intersection[{u,v}]++;
    }
    debug(intersection)
