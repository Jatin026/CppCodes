...
void solve(){
    ...
    Matrix A ;
    A.a[0][1]=b.size();
    A.a[1][0]=a.size(); A.a[1][1]=a.size();
    cin >> q;
     
    while (q--)
    {
        ll w ; cin >> w;
        Matrix R = expo_power(A,w-1);
        if(w==1) cout<<a.size()+b.size()<<nline;
        else{
            cout<< (a.size()*R.a[0][1]+ (b.size()*R.a[1][0]) + a.size()*R.a[1][1] + b.size()*R.a[0][0])%mod<<nline;
        }
    }
