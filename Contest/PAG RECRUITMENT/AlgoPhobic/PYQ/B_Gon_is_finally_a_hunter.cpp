...
ll ans(ll i ,  ll train){
    ...
    if(dp.count(a)) return dp[a];
        ...
        while (moves--)
        {
            string s = to_string(currNen);
            ll sumofdig=0;
            for (int i = 0; i < s.size(); i++)
            {
                sumofdig+=s[i]-48;
            }
                currNen+=sumofdig*sumofdig;
            }
        if(train>=100){
            return  dp[a]=currNen*xp[i]+ans(i+1,train);
        }
        return dp[a]=max(ans(i+1,train+1),currNen*xp[i]+ans(i+1,train));
    }
}
void solve(){
    cin>>n>>x;
