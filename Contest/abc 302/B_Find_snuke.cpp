...
void solve(){
    ...
    vector<string> v(n);
    for(auto &x : v) cin >> x;
    // vvi vis(n,vi(m)),par(n,vi(m,-1));
    vector<char> pos{'s', 'n', 'u'  , 'k'  ,'e'};
    vector<vpr> ind(26);
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < m; j++)
        {
            ind[v[i][j] - 'a'].emplace_back(i,j);
        }
    }
    for(auto x : ind['s' - 'a']){
        if(x.second+4<m){
            ...
            for (int i = 0; i < 5; i++)
            {
                if(pos[i] != v[x.first][x.second+i]) flag = 0;
            }
            if(flag){
                for (int i = 0; i < 5; i++)
                {
                    cout << x.first + 1 << " " << x.second + i + 1 << nline;
                }
                return;
            }
        }
        ...
        if(x.first+4<n){
            ...
            for (int i = 0; i < 5; i++)
            {
                if(pos[i] != v[x.first+i][x.second]) flag = 0;
            }
            if(flag){
                for (int i = 0; i < 5; i++)
                {
                    cout << x.first + 1 +i << " " << x.second  + 1 << nline;
                }
                return;
            }
        }
        ...
        if(x.second>= 4 && x.first >= 4){
            bool flag = 1;
            for (int i = 0; i < 5; i++)
            {
                if(pos[i] != v[x.first-i][x.second-i]) flag = 0;
            }
            if(flag){
                for (int i = 0; i < 5; i++)
                {
                    cout << x.first + 1 -i << " " << x.second - i + 1 << nline;
                }
                return;
            }
        }
        if(x.first+4<n && x.second+4 < m){
            bool flag = 1;
            for (int i = 0; i < 5; i++)
            {
                if(pos[i] != v[x.first+i][x.second+i]) flag = 0;
