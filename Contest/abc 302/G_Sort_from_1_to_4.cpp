...
void solve(){
    ...
    for (int i = 0; i < 4; i++)
    {
        for (int j = i+1; j < 4; j++)
        {
            int req = min(edge[i][j] , edge[j][i]);
            edge[i][j] -= req; edge[j][i] -= req;
            ans += req;
        }
    }
    ...
    for (int k = 0; k < 4; k++)
    {
        for (int i = 0; i < 4; i++)
        {
            for (int j = 0; j < 4; j++)
            {
                int req = min(edge[i][k] , edge[k][j]);
                req = min(req,edge[j][i]);
                ans += 2*req;
                edge[i][k] -= req;    edge[k][j] -= req;    edge[j][i] -= req;  
            }
    ...
    for (int l = 0; l < 4; l++)
    {
        for (int k = 0; k < 4; k++)
        {
            for (int i = 0; i < 4; i++)
            {
                ...
                {
                    int req = min(edge[i][k] , edge[k][j]);
                    req = min(req,edge[j][l]);
                    req = min(req,edge[l][i]);
                    ans += 3*req;
                    edge[i][k] -= req;    edge[k][j] -= req;    edge[j][l] -= req , edge[l][i] -= req;  
                }
