...
void solve(){
    ...
    function<void(int,int)> create = [&](int v , int p){
        for(auto u : adj[v]){
            if(u == p) continue;
            par[u] = v;
            create(u,v);
            
            order[v].insert(mk(-sz[u],u));
            sz[v] += sz[u];
            sum[v] += sum[u];
        }
        sum[v] += imp[v];
    ...
    for (int i = 0; i < m; i++)
    {
        ...
        if(type){
            ...
            int heavy = (*order[x].begin()).second;
            order[par[x]].erase(mk(-sz[x],x));
            order[x].erase(order[x].begin());
            sz[x] -= sz[heavy];
            sz[heavy] += sz[x];
            sum[x] -= sum[heavy];
            sum[heavy] += sum[x];
            order[heavy].insert(mk(-sz[x],x));
            order[par[x]].insert(mk(-sz[heavy],heavy));
            par[heavy] = par[x];
            par[x] = heavy;
        }
        else{
