...
void solve(){
    ...
    function<void(int,int)> dfs = [&](int v , int p) {
        ...
        sum[v] += depth[v];
        sum[v] %= mod;
        dp[v][0] += BinExpItr(h[sz[v]],h[depth[v]],mod);
        dp[v][0] += BinExpItr(h[depth[v]],h[sz[v]],mod);
        dp[v][0] %= mod;
        dp[v][1] += BinExpItr(h[sz[v]],h[depth[v]],mod2);
        dp[v][1] += BinExpItr(dp[v][1],sum[v],mod2);
        dp[v][1] %= mod2;
        dp[v][1] += BinExpItr(h[depth[v]],h[sz[v]],mod2)*sum[v];
        dp[v][1] %= mod2;
    };
    dfs(0,0);
    bool ans = 1;
    set<int> axis;
    function<void(int,int)> find_axis = [&](int v , int p) {
        map<ll,ll> f,rev;
    ...
    find_axis(0,0);
    debug(ans)
    function<void(int,int)> find_axis2 = [&](int v , int p) {
        map<ll,ll> f,rev;
