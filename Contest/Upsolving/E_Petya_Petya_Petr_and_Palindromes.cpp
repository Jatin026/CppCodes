...
void solve(){
    ...
    ll ans = 0;
    for(int i =  k/2; i < n ; i++){
        auto it = lb(all(h[i&1][v[i]]),max(i-k+1,2*(k/2) - i )) ;
        auto it2 = ub(all(h[i&1][v[i]]),min(i-2,2*(n-k/2 ) - i));
        int up = min(2*(n-k/2 )-i  , i-2) , down = max(i-k-1, 2*(k/2) - i );
        if(down > up) continue;
        up++;
        ans += (up - down)/2 - (it2  - it);
