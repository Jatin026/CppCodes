...
const int N = 5005;
vector<vector<vector<ll>>> dp(N,vector<vector<ll>> (N,vector<ll> (4,1e17)));
void solve(){
    int p1 , p2 , h , s;
    ll t1 , t2 ;
    cin >> p1 >> t1 >> p2 >> t2 >> h >> s;
    ...
    for (int move = 1; move < N; move++)
    {
        ...
        {
            dp[move][health][0] = min(dp[move][health][0] , dp[move-1][health][0]);
            dp[move][max(0,health - (p1-s) )][1] = min(dp[move][health][1] , dp[move-1][health][0]);
        }
