...
const int N = 105 , M = 1e4 + 120 , inf = 1e9;
// const int ZERO = 2e4;
// vector<vector<lld>> dp(N,vector<lld> (M,-1e9));
// void solve(){
//     int n;
//     cin >> n;
//     dp[0][0] = 0;
...
//         int a , b;
//         cin >> a >> b;
//         // vector<vector<lld>> new_dp(dp);
//         // for (int glass = 0; glass <= n; glass++)
//         // {
//         //     for (int rem = -ZERO ; rem < M; rem++)
//         //     {
//         //         int take = min(b,rem-b + ZERO);
//         //         if(rem - b + ZERO >= 0 && rem + ZERO < M) new_dp[glass][rem - b + ZERO] = max(new_dp[glass][rem-b + ZERO] , dp[glass][rem + ZERO] + b/2);
//         //         if(rem - b + ZERO < 0 && rem + ZERO < M) new_dp[glass][0] = max(new_dp[glass][0] , dp[glass][rem + ZERO] + (b-rem-ZERO)/2);
//         //         if(ZERO + rem + 2*(a-b) < M && glass < n) new_dp[glass+1][ZERO + rem + 2*(a-b)]  = max(new_dp[glass+1][ZERO + rem + 2*(a-b)] , dp[glass][ZERO + rem] + b);      
//         //     }
//         // }
//         // swap(new_dp,dp);
//         // for (int j = 0; j < 10; j++)
...
void solve(){
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        b_tot += b;
        vvi new_dp(dp);
        for (int glass = 0; glass < n+1; glass++)
        {
            for (int rem = 0; rem < M; rem++)
            {
                if(rem + a < M && glass + 1 <= n){
                    new_dp[glass+1][rem+a] = max(new_dp[glass+1][rem+a] , dp[glass][rem] + b);
                }
            }
        }
        swap(new_dp,dp);
    ...
    for (int glass = 1; glass <= n; glass++)
    {
        ...
        {
            if(dp[glass][wt] > -1){
                int taken = max(0,wt - dp[glass][wt]);
                int left = b_tot - dp[glass][wt];
                ans = max(ans ,(lld) dp[glass][wt] + min(taken*1.0 , left/2.0));   
            }
        }
        cout << ans << " ";
    }
