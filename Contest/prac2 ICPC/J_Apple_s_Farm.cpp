...
void solve(){
    ll n , m , k;
    cin >> n >> m >> k;
    vector<vll> val(n,vll(m));
    for(auto &x : val) for(auto &y : x) cin >> y;
    vector<vector<map<ll,ll>>> dp(n,vector<map<ll,ll>>(m));
    // function<map<ll,ll>(int, int)>  rec = [&] (int i , int j) {
    //     if(i == 0 && j == 0){
    //         map<ll,ll> cnt;
    //         cnt[val[0][0]] = 1;
    //         return cnt;
    ...
    //     map<ll,ll> cnt;
    //     if(i>=1){
    //         map<ll,ll> temp = rec(i-1,j);
    //         for(auto &x : temp){
    //             cnt[(x.first+val[i][j])%k] += x.second;
    //         }
    //     }
    //     if(j>0){
    //         map<ll,ll> temp = rec(i,j-1);
    //         for(auto &x : temp){
    //             cnt[(x.first+val[i][j])%k] += x.second;
    //         }
    //     }
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < m; j++)
        {
            if(i>=1){
                for(auto x : dp[i-1][j]){
                    dp[i][j][(val[i][j]+x.first)%k] += x.second; 
                }
            }
            if(j>=1){
                for(auto x : dp[i][j-1]){
