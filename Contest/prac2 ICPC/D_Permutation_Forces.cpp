...
void solve(){
    ...
    for (int i = 2; i < N; i++)
    {
        ...
        for (int dx = -1; dx < 2; dx++)
        {
            ...
            {
                if(abs(dx) + abs(dy) == 1 && valid(loc[i].ff+dx,loc[i].ss+dy)){
                    if(vis[loc[i].ff+dx][loc[i].ss+dy]) d.merge(compress[loc[i].ff+dx][loc[i].ss+dy] , compress[loc[i].ff][loc[i].ss]);    
                }
            }
            
        }
        vis[loc[i].ff][loc[i].ss] = 1;
        if(d.size(compress[loc[1].ff][loc[1].ss] ) == i ) ans = i;
    }
