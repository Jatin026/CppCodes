...
void solve(){
    ll n , k;
    cin >> n >> k ;
    vector<vll> pref(n+1,vll(31)), suff(n+1,vll(31));
    vector<vll> contri(n+3,vll(31)) , suff_contri(n+1,vll(31));
    ...
    for (int bit = 0; bit < 30; bit++)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            pref[i+1][bit] = pref[i][bit];
            if(!freq.empty()){
                if(freq.back() < i) freq.pop_back();
                if(!freq.empty()){
                    pref[i+1][bit] += (1ll<<bit)*(n-freq.back());
                    contri[i+1][bit] = (1ll<<bit)*(n-freq.back());
                }
            }
        ...
        for (int i = 0; i < n; i++)
        {
            if(!freq.empty()){
                if(freq.back() < i) freq.pop_back();
                if(!freq.empty()){
                    suff_contri[n-i-1][bit] = (1<<bit)*(n-freq.back());
                }
            }
        }
        ...
        for (int i = 0; i < n; i++)
        {
            suff[i+1][bit] = suff[i][bit] + suff_contri[i][bit];
    ...
    auto check = [&](ll l , ll r) -> bool {
        ...
        for (int bit = 0; bit < 30; bit++)
        {
            if(!pref[n][bit]) continue;
            ll val = pref[n][bit] - pref[min(n,r+1)][bit];
             
            val += suff[l][bit];
            debug(cum)
           // debug(val)
            if(prev[l][bit] != -1 && r < n && l > 0){
                ll ind = prev[l][bit]+1;
                
                cum +=(l-ind)*(contri[r+2][bit]); 
            ...
            else cum += (l)*(contri[r+2][bit]);
            // debug(cum)
            // debug(bit)
            debug(pref[n][bit] - pref[min(n,r+1)][bit])
            // if(val > k) return 0;
            cum += val;
            // if(cum > k) return 0;
        }
