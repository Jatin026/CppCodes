...
void solve(){
    ...
    while(!q.empty()){
        ...
        for (int i = 0; i < 4; i++)
        {
            if(valid(s.first+dx[i],s.second+dy[i]) && !vis[s.first+dx[i]][s.second+dy[i]] && v[s.first+dx[i]][s.second+dy[i]] == '0' ){
                debug(s)
                q.push(make_pair(s.first+dx[i],s.second+dy[i]));
                vis[s.first+dx[i]][s.second+dy[i]] = 1;
            }
        }
    }
    debug(vis)
