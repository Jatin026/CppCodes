...
void solve(){
    ...
    {
        dp[j+1] = dp[j];
        for (int i = 0; i+av[j] <= rem[0]; i++)
        {
            dp[j+1][av[j]+i] =(dp[j+1][av[j]+i]|dp[j][i]);
        }      
    }
    map<int,int> used;
    int val = 0;
    ...
    debug(used)
    fill(all(dp),vector<bool> (n+1,0));
    dp[0][0] = 1;
    for (int j = 0; j < av.size(); j++)
    {
        dp[j+1] = dp[j];
        if(used[av[j]]) continue;
        for (int i = 0; i+av[j] <= rem[1]; i++)
        {
            dp[j+1][av[j]+i] =(dp[j+1][av[j]+i]|dp[j][i]);
        }      
    }
