...
void solve(){
    ...
    int index = 0;
    for(auto &x : comp) x.second = index++;
    for(auto &x : v) x = comp[x];
    vvi ind(n);
    // for(int i = 0;  i < n ; i++) ind[v[i]].pb(i);
    // vector<vector<ll>> dp(n , vector<ll> (2,INF));
    // // int preva = ind[0].front();
    // for(int i = 0 ; i < ind[0].size(); i++){
    //     int x = ind[0][i]  , preva = ind[0][(i-1+ind[0].size())%ind[0].size()] , ;
    //     dp[x][0] = abs(x-s) + min(n - 1 - x + preva , 2*(ind[0].back() - x) + (x - ind[0].front()));
    //     dp[x][1] = abs(x-s) + (ind[0].back() - x) + 2*(x - ind[0].front());
    // }
    // debug(dp)
    // for(int i = 1; i < n; i++){
    //     for(auto x: ind[i]){
    //         for(auto y : ind[i-1]){
    //             dp[x][0] = min(abs(x-ind[i-1].front()) + 2*(ind[i].back() - x) + (x - ind[i].front()) + dp[y][0],dp[x][0]);
    //             dp[x][0] = min(dp[x][0],abs(x-ind[i-1].back()) + 2*(ind[i].back() - x) + (x - ind[i].front()) + dp[y][1]) ;
    //             dp[x][1] = min(abs(x-ind[i-1].front()) + (ind[i].back() - x) + 2*(x - ind[i].front()) + dp[y][0],dp[x][1]);
    //             dp[x][1] = min(dp[x][1] , abs(x-ind[i-1].back()) + (ind[i].back() - x) + 2*(x - ind[i].front()) + dp[y][1]) ;
    //         }
    //     }
    // }
    // debug(dp)
    ...
    // cout << ans;
    /*
        dis[i][j] -> start at i finish at j , visit each member of the group once -> minimum time
        dp[j] -> last cell to visit of this group was j, find the minimum time to reach here.
        dp[j] = mini[k] + short_dis(k , i) + dis[i,j]
