...
class Employee{
    ...
    public:
        ...
        Employee(){
            cin >> this->name;
            cin >> this->skill_count;
            loop(i,0,skill_count,1){
                string tmp;
                int tmp_count;
                cin >> tmp >> tmp_count;
                skills[tmp] = tmp_count;
            }
        };

        void print(){
            cout << name << " " << skill_count << "\n";
            for(auto &i: skills){
...
class Project{
    private:
        string name;
        int duration, score, best_before, employee_needed;
        map<string, int> skills;

    public:
        Project(){
            cin >> name >> duration >> score >> best_before >> employee_needed;
            loop(i,0,employee_needed,1){
                string tmp_skill;
                int tmp_level;
                cin >> tmp_skill >> tmp_level;
                skills[tmp_skill] = tmp_level;
            }
        }

        void print(){
            cout << name << " " << duration << " " << score << " " << best_before << " " << employee_needed << "\n";
            for(auto &i: skills){
                cout << i.first << " " << i.second << "\n";
            }
        }
        friend bool operator < (Project &a , Project &b) ;
};
class Inventory{
    public:
        map<string, map<int, set<string>>> resources;

        Inventory(vector<Employee> &vec){
            for(auto &emp: vec){
                for(auto &skill: emp.skills){
                    resources[skill.first][skill.second].insert(emp.name);
                }
            }
        }
        ...
        void remove(Employee &emp){
            for(auto &skill: emp.skills){
                resources[skill.first][skill.second].erase(emp.name);
            }
        }

        void update(Employee &emp){
            //Update emp before updating here
            for(auto &skill: emp.skills){
                resources[skill.first][skill.second].insert(emp.name);
            }
        }
...
{   
    ...
    if(a.best_before > b.best_before) return false;
    if(a.best_before < b.best_before) return true;
...
void solve(){
    ...
    int employees, projects;
    cin >> employees >> projects;
    vector<Employee> emp_arr(employees);
    vector<Project> proj_arr(projects);
    sort(all(proj_arr));
    for(auto x : proj_arr) x.print();
    Inventory inventory(emp_arr);
    for(auto x : proj_arr){
        bool req = false;
    }
