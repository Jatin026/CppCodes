...
void solve(){
    ...
    it--;
    auto ind = it - pref.begin();
    Mint expA{}, tmpA{};
    for(auto i{0}; i < n; i++){
        expA = (2*expA + arr[i] + 1)/2;
    }
    for(auto i{0}; ind; i++){
        tmpA = (2*tmpA + arr[i] + 1)/2;
    }
    for(auto i{pref[ind]}; i <= a; i++){
        tmpA += ((Mint)i)/(pref[ind+1] - pref[ind]);
    }
    tmpA += kA*pref[n];
    expA += (kA*(kA+1))/2*pref[n];
    int kB = b / pref[n];
    b = b % pref[n];
    it = upper_bound(all(pref), b);
    it--;
    ind = it - pref.begin();
    Mint expB{}, tmpB{};
    for(auto i{0}; i < n; i++){
        expB = (2*expB + arr[i] + 1)/2;
    }
    for(auto i{0}; ind; i++){
        tmpB = (2*tmpB + arr[i] + 1)/2;
    }
    for(auto i{pref[ind]}; i <= a; i++){
        tmpB += ((Mint)i)/(pref[ind+1] - pref[ind]);
    }
    tmpB += kB*pref[n];
    expB += (kB*(kB+1))/2*pref[n];
    cout << expB + tmpB - expA - tmpA << "\n";
}
