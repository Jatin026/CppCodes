...
void solve(){
    ...
    //     for (int j = 1; j < i; j++)
    //     {
    //         Mint k = BinExpItr(2,i-j);
    //         tot += abs(v[i-1].first-v[j-1].first)/k;
    //     }
    //     //tot += dp[i];
    // }
    // Mint d = 2;
    ...
    long long max_val = v[0].first;
    for (int i = 1; i < n; i++) {
        max_val = max(max_val, (long long) v[i-1].first);
        dp[i] = dp[i-1] + (max_val - v[i].first) * (1LL << (i-1));
    }
    max_val = v[n-1].first;
    long long sum = 0;
    for (int j = n-2; j >= 0; j--) {
        max_val = max(max_val, (long long) v[j+1].first);
        sum += (v[j].first - max_val) * (1LL << j);
    }
    sum += dp[n-1];
    cout << sum << endl;
