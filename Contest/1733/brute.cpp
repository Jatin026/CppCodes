...
int n=4;
ll rec(int i , int x , int y , string a , string b){
    if(a==b) return 0;
    if(i==n) return 1e9;
    ll ans=1e9;
    if(a[i]==b[i]) return rec(i+1,x,y,a,b); 
    for (int j = i+1; j < a.size(); j++)
    {
        if(a[j]!=b[j]){
            if(j!=i && abs(j-i)==1){
                ...
                ans=min(ans,x+rec(i+1,x,y,temp,b));
            }
            else if(j!=i){
                string temp=a;
                if(temp[j]=='0') temp[j]='1';
                else temp[j]='0';
                ans=min(ans,y+rec(i+1,x,y,temp,b));
            }
        }
    }
...
void solve(){
    for (int i = 0; i < 5; i++)
    {
        for (int j = i; j < 5; j++)
        {
            string a=binarystring(4,i);
            string b= binarystring(4,j);
            cout<<a<<" "<<b<<nline;
            cout<<rec(0,5,3,a,b)<<nline;
            cout<<nline;
        }
