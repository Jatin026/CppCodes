...
void solve(){
    ...
    vvi vis(val);
    for(auto &x : val) for(auto &y : x) cin >> y;
    queue<pr> source;
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < m; j++)
        {
            if(i+1 < n && j+1 < m ){
                ...
                for (int k = 0; k < 2; k++)
                {
                    ...
                    {
                        if(s.empty()){
                            s.push_back(val[i+k][j+lk]);
                        }
                        else{
                            if(s.back() != val[i+k][j+lk]) s.push_back(val[i+k][j+lk]);
                        }
                    }
        ...
        for (int i = 0; i < 2; i++)
        {
            for (int j = 0; j < 2; j++)
            {
                if(valid(x+i,y+j)){
                    cnt++;
                    if(val[x+i][y+j]) {
                        if(s.empty()) s.push_back(val[x+i][y+j]);
                        else{
                            if(s.back() != val[x+i][y+j]) s.push_back(val[x+i][y+j]);
                        }
                    }
                }  
            }
        }
        if(cnt == 4 && !s.empty() && s.size() == 1){
            int c = (s.front());
            ans.emplace_back(array<int,3> {x+1,y+1,c});
            for (int i = 0; i < 2; i++)
            {
                for (int j = 0; j < 2; j++)
                {
    ...
    while(!source.empty()){
        int x = source.front().first , y  = source.front().second;
        source.pop();
        check(x,y);
        check(x-1,y);
        check(x-1,y-1);
        ...
        for (int dx = -1; dx < 2; dx++)
        {
            ...
            {
                if(abs(dx) + abs(dy) == 1 && valid(x+dx,y+dy) && !vis[x+dx][y+dy] && !val[x+dx][y+dy]){
                    source.push(mk(x+dx,y+dy));
                    vis[x+dx][y+dy] = 1;
                }
            }
