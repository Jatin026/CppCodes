...
struct segtree{
    ...
    void build(vector<int> &a,int x , int lx , int rx){
        if(rx-lx==1){
            if(lx<(int)a.size()){
                sums[x]= (a[lx] >= z);
                if(a[lx] >= z) val[x] = a[lx];
                if(a[lx] < z) sums[x] = -1e9;
                if(a[lx] == z){
                    pref[x]=max(pref[x],(1ll)*(a[lx] >= z));
                    suff[x]=max(suff[x],(1ll)*(a[lx] >= z));
                    ans[x]=max(ans[x],(1ll)*(a[lx] >= z));
                }
            }
            return;
        }
    ...
    ll maxSubsegment(int l , int r){
        return maxSubsegment(l,r,0,0,size);
    }
};
void solve(){
    int n,m;
    ...
    for (int i = 0; i < m; i++)
    {
        ...
        if(type == 1) {
            ...
            cout << v[z].maxSubsegment(l,r) << nline;
        }
    }
