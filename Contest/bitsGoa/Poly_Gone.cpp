...
void solve(){
     int n;
        ...
        sort(all(v));
        vector<Mint> use(5002), useless(5002);
        for(auto a: v){
            vector<Mint> x = use , y = useless;
            for (int i = int(a+1); i < 5002; i++)
            {
                x[min((5001), int(i + a))] += use[i];
            }
            x[int(a)]++;
            for (int i = 0; i < int(a+1); i++)
            {
                y[min(5001, int(i + a))] += use[i];
            }
            for (int i = int(a+1) ; i < 5002; i++)
            {
                x[min(5001, int(i + a))] += useless[i];
            }
            for (int i = 0; i < int(a+1); i++)
            {
                y[min(5001, int(i + a))] += useless[i];
            }
            swap(x,use);
            swap(useless,y);
        }
