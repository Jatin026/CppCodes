...
void solve(){
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if(i != n-1){
            int j = lg(n-1 - i );
            int maximum = max(st[j][i+1], st[j][n-1 - (1 << j) + 1]);
            ans = min(ans , abs(maximum-v[i].first));
            debug(ms)
            if(!ms.empty()){
                auto it = ms.lb(v[i].first);
                if(it != ms.end() && (*it) >= maximum) ans = min(ans , abs((*it)-v[i].first));  
                if(it != ms.begin()){
                    --it;
                    if((*it) >= maximum)
                        ans = min(ans , abs((*it)-v[i].first));
                }
            }
        }
        else{
