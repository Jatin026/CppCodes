...
void solve(){
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = i+1; j < n; j++)
        {
            auto it = ub(v.begin() + i + 1, v.begin() + j , (v[i]+v[j])/2);
            if(it != v.begin() + j){
                int val = (*it);
                if(it != v.begin() + j)
                    ans = min(ans ,abs(v[i]+v[j] - 2*val) );
                --it;
                if(it >= v.begin() + i + 1){
                    int val = (*it);
                    ans = min(ans ,abs(v[i]+v[j] - 2*val) );
            ...
            it = ub(v.begin()+j+1 , v.end() , 2*v[j] - v[i] );
            if(it != v.end()){
                int val = (*it);
                if(it != v.end())
                    ans = min(ans , abs(val + v[i] - 2*v[j]));
                --it;
                if(it >= v.begin() + j + 1){
                    val = (*it);
                    ans = min(ans , abs(val + v[i] - 2*v[j]));
            ...
            if(i == 0) continue;
            if(v[j] - 2*v[i] >= 0 ) ans = min(ans , v[j] - 2*v[i] + v[0]);
            else {
                it = ub(v.begin() , v.begin() + i , 2*v[i] - v[j] );
                int val = (*it);
                if(it != v.begin() + i)
                    ans = min(ans  , abs(val+v[j] - 2*v[i]));
                if(it != v.begin()){
                    --it;
                    val = (*it);
                    ans = min(ans  , abs(val+v[j] - 2*v[i]));
