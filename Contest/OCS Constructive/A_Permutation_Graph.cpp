...
void solve(){
    ...
    segtreeMax maxi;
    segtreeMin mini;
    maxi.init(n+2); mini.init(n+2);
    maxi.build(v); mini.build(v);
    vi nextS(n,n), nextG(n,n);     
    stack<int> s , g;
    for(int i = n-1 ; i > -1 ; i--){
        while(!g.empty() && (g.top()) < v[i]){
            g.pop();
        }
        if(!g.empty()) nextG[i] = ind[g.top()];
        while(!s.empty() && (s.top()) > v[i]){
            s.pop();
        }
        if(!s.empty()) nextS[i] = ind[s.top()];
        s.push(v[i]); g.push(v[i]);
    } 
    int l = 0 , loc = 0 , ans = n , move = 1;
    while(l < n - 1){
        if(move){
            debug(maxi.sum(l,nextS[l]))
            l = ind[maxi.sum(l,nextS[l])];
        }
        else{
            debug(l)
            debug(nextG[l])
            debug(mini.sum(l,nextG[l]))
            l = ind[mini.sum(l,nextG[l])];
        }
        move ^= 1;
        loc++;
        if(loc > n) break;
