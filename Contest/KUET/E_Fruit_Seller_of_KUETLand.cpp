...
void solve(){
    ...
    cin >> n >> m >> r >> q;
    --r;
    vi nutirtion(n) , price(n);
    vvi adj(n);
    for(int i = 0; i < n; i++){
        cin >> price[i] >> nutirtion[i];
    }
    for(int i = 0; i < m ; i++){
        int a , b;
        cin >> a >> b;
    ...
    vi sub_price(n) ,  sub_nutri(n);
    function<void(int , int)> dfs = [&](int v , int p){
        for(auto u : adj[v]){
            if(u == p) continue;
            dfs(u,v);
            sub_nutri[v] += sub_nutri[u];
            sub_price[v] += sub_price[u];
        }
        sub_price[v] += price[v];
        sub_nutri[v] += nutirtion[v];
    };
    const int W  = 3005;
    dfs(r,r);
    vvi ans(n , vi(W));
    debug(sub_nutri)
    function<vi(int , int)> calc = [&](int v , int p){
        vi res(W);
        res[sub_price[v]] = sub_nutri[v];
        for(int i = 0; i + sub_price[v] < W; i++){
            ans[i + sub_price[v]] = max(ans[i + sub_price[v]] , ans[i] + sub_nutri[v]);
        }
        for(auto u : adj[v]){
            if(u == p) continue;
            calc(u,v);
