...
void solve(){
    ...
    {   
        ...
        cin >> a >> b >> t;
        --a; --b;
        adj[a].emplace_back(array<ll,3> {b,t,0});
        adj[b].emplace_back(array<ll,3> {a,2*t,1});
    }
    vector<vll> dist(n , vll(N , INF));
    multiset<array<ll,3>> ms;
    dist[s][0] = 0;
    ms.emplace(array<ll,3> {0 , s , 0});
    while(!ms.empty()){
        auto v = *ms.begin();
        ms.erase(ms.begin());
        for(auto u : adj[v[1]]){
            if(v[2] + u[2] <= k && dist[u[0]][v[2] + u[2]] > v[0] + u[1]){
                auto it = ms.find(array<ll,3> {dist[u[0]][v[2]] , u[0] ,v[2] + u[2]});
                if(it != ms.end()) ms.erase(it);
                dist[u[0]][v[2] + u[2]] = v[0] + u[1];
                ms.insert(array<ll,3> {v[0] + u[1] , u[0] , v[2] + u[2]});
            }
    ...
    while(q--){
        ...
        cin >> x;
        --x;
        if(ans[x] == INF) cout << -1 << nline;
        else cout << ans[x] << nline;
    }
}
int main(){
    FAST
