...
void solve(){
    int n;
    cin >> n;
    vector<vi> st(0,vi(2,0));
    vll dpl(n+1) , dpr(n+1);
    vll suff(n+1,0) , pref(n+1,0);
    vi v(n);
    ll sum = 0;
    for(auto &x : v){
        cin >> x;
    ...
    {
        pref[i+1] = pref[i] + v[i];
        suff[i+1] = suff[i] + v[n-i-1];
    }
    // reverse(all(suff));
     
    /*
        maximize the amount of damage in a single explosion
        SUM(summation of height) - damage
        vj - j <= vi - i (break) <- dp[j] use
        v[j] - j > vi - i -> v[j] = v[i] - i
        1 3 5 5
        1 2 3 4


        5 ? 
        maximum damage that i can inflict if i choose i as the explosion point.
    ...
    */
    ...
    {
        int j = max(0,i-v[i-1]);
        while(!st.empty() && st.back()[1] > v[i-1] - i){
            st.pop_back();
        }
        if(!st.empty())  j = max(j , st.back()[0]);
        debug(st)
        st.push_back({i,v[i-1]-i});
        debug(j)
        ll len = (i-j);
        if(v[i-1] > len) dpl[i] = dpl[j] + (len*v[i-1] - len*(len-1)/2) ;
        else dpl[i] =  (len*v[i-1] - len*(len-1)/2) ;
    }
    reverse(all(v));
    ...
    for (int i = 1; i <= n; i++)
    {
        ...
        if(v[i-1] > len) dpr[i] = dpr[j] + (len*v[i-1] - len*(len-1)/2) ;
        else dpr[i] = (len*v[i-1] - len*(len-1)/2) ;
    }
    ll ans = 1e15;
    reverse(all(v));
    for (int i = 1; i <= n; i++)
    {
        ans = min(ans , sum + 2*v[i-1] - dpl[i] - dpr[n-i+1]);
        debug(ans)
    }
    debug(nline)
    cout << ans << nline;
