...
void solve(){
    ...
    int mini[K + 1][N+7],maxi[K + 1][N+7];
    std::copy(array.begin(), array.end(), maxi[0]);
    std::copy(array.begin(), array.end(), mini[0]);
    for (int i = 1; i <= K; i++)
    ...
    while(L<R){
        ...
        int minimum = min(mini[i][L], mini[i][R - (1 << i) + 1]);
        int maximum = max(maxi[i][L], maxi[i][R - (1 << i) + 1]);
        if(max(array[L] , array[R]) != maximum){
            if(min(array[L] , array[R]) != minimum){
                cout << L+1 << " " << R+1 << nline;
                return;
            }
            else{
                if(array[L] == minimum) L++;
                else R--;
            }
        }
        else{
            if(array[L] == maximum) L++;
            else R--;
        }
    }
    cout << -1 << nline;
