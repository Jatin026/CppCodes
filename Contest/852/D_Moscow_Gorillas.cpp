...
void solve(){
    ...
    auto union_range = [&](pr p , int l , int r) -> pr{
        if(p == make_pair(-1,-1)){
            return (make_pair(l,r));
        }
        else{
            return (make_pair(min(p.ff,l),max(r,p.ss)));
        }
    };
    auto range_intersection = [&](pr p , int l , int r) -> ll {
        if((p.ss >= l && p.ff <=l) || (p.ff <=r && p.ss >=r) || (p.ff ==  l || p.ss ==r) ){
            return 0;
        }
        else{
            if(p.ff > l && p.ss < r){
                ll left = p.ff - l , right = r - p.ss;
                return (left*right);
            }
            else if(p.ff > r){
                ll left = p.ff - r , right = n - p.ss;
                assert(left*right >= 0);
                debug(p)
                debug(l)
                debug(r)
    ...
    for (int i = 0; i < n; i++)
    {
        ll l = min(inda[i],indb[i]) , r = max(inda[i],indb[i]) ;
        if(range == make_pair(-1,-1)){
            ans += (l*(l+1))/2  + (n-r-1)*(n-r)/2 + (r-l-1)*(r-l)/2;
        }
        else ans += range_intersection(range,l,r);
         
        range  =  union_range(range,l,r);
        debug(i)
        debug(ans)
        debug(range)
        debug(l) debug(r)
