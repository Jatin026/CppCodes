...
void solve(){
    ...
    ll sum = 0;
    set<pr> s;
    for(auto &x : range) cin >> x.first;for(auto &x : range) cin >> x.second;
    for(auto x : range){
        if(x.first == x.second) s.insert(mk(x.first,x.first));
    ...
    if(sum < k){
        ...
        for (int i = 0; i < n; i++)
        {
            sum += range[i].second - range[i].first + 1;
            if(range[i].first ==  range[i].second) one++;
            cost += 2;
            if(sum >= k){
                sum -= range[i].second - range[i].first + 1; 
                r = (k - sum) + range[i].first - 1;
                break;
            }
        }
        debug(r)
        ...
        vi seg(n*10+100);
        for(auto x : range){
            // if(x.first == x.second && x.first > r ) one++;
            if(x.first >= r && x.first < r + n*10+100){
                seg[x.first-r]++;
            }
            else if(x.first < r && x.second >= r ){
                seg[0]++;
            }
            if(x.second >= r && x.second + 1 < r + n*10+100 ){
                seg[x.second+1 - r]--;
            }
        }
        ll curr = 0;
