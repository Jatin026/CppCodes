...
vector < vector < int > > L ( 100010 , vector < int >  ( 0 ) ) ;

vector < vector < int > > W ( 100010 , vector < int >  ( 0 ) ) ;
...
void dfs ( int a )
{
    ...
    long long z , o , i ; z = 0 ; o = 0 ; i = 0 ;

    for ( int j = 0 ; j < S[a]; ++ j )
    {
       

        if ( V[L[a][j]] == 0 ) dfs(L[a][j]) ;
