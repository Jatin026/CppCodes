...
void solve(){
    ...
    // cin >> n;
    vi a(n) , d(n);
    for(auto &x : a)  x = Input::read_int();
    for(auto &y : d) y = Input::read_int();
    priority_queue<pair<ll,ll> , vector<pair<ll,ll>> , greater<pair<ll,ll>>>  ms;
    vector<vector<pair<int,int>>> qry(n);
    for(int i = 0; i < n; i++){
        if(a[i]){
            qry[max(0,i-d[i])].emplace_back(min(n-1,i+d[i]),a[i]);
        }
    }
    // cout << "size: " << ms.size() << "\n";
    auto check = [&](ll maxi) -> bool {
        // cout << maxi << "\n";
        auto temp = ms;
        vector<ll> val(n);
        for(int i = 0; i < n; i++){
            ...
            if(!temp.empty() && temp.top().first < i) {
                // cout << "bhag bsdk :)\n";
                // cout << i << " " << (*temp.begin())[0] << " " << (*temp.begin())[1] << " " << (*temp.begin())[2] << nline;
                return 0;
            }
            while(!temp.empty() && val[i] < maxi){
                auto it = (temp.top());
                // cout << i << " " << it[0] << " " << it[1] << " " << it[2] << nline;
                if(it.first < i) return 0;
                temp.pop();
                if(it.second + val[i] > maxi){
                    temp.push({it.first,it.second+val[i] - maxi});
                    val[i] = maxi;
                }
