...
void solve(){
    ...
    for (int j = n-1 , cnt = 0; j > -1 && cnt < 10 ; j-- , cnt++)
    {
        vector<set<int>> temp = adj;
        vi small(n,inf);
        vi p(n,-1);
        function<void(int,int)> dfs = [&](int v,int par){
            for(auto u : adj[v]){
                ...
                p[u] = v;
                dfs(u,v);
                small[v] = min(small[v],small[u]);
            }
            if(small[v] == inf) small[v] = v;
        };
        ...
        auto cmp = [&](int &a , int &b) -> bool {
            if(small[a] < small[b] ) return 1;
            else if(small[a] > small[b]) return 0;
            else return (a < b);
        };
    
        for (int i = 0; i < n; i++)
        {
            if(adj_new.size() > 1) sort(all(adj_new[i]),cmp);
        }
        
        function<void(int,int)> dfs_new = [&](int v,int par){
            for(auto u : adj_new[v]){
                if(u == par) continue;
                dfs_new(u,v);
            }
            ans.pb(v);
