...
void solve(){
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        vpr edges;
        for(auto u : adj[i]){
            if(!s.empty() && !s.count(u)) edges.emplace_back(u,i);
            if(edges.size() == 2) break;
        }
        
        if(edges.size() == 2){
            cout << "YES\n";
            edges.emplace_back(i,ans.front());
            edges.emplace_back(i,ans.back());
            debug(ans)
            for (int j = 0; j < (int)ans.size() - 1; j++)
            {
                edges.emplace_back(ans[j],ans[j+1]);
            }
