...
void solve(){
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        sum += a;
        mp[mk(a,b)] = i+1;
        ele.emplace_back(array<int,3> {a,b,i+1});
    }
    vi s(n);
    sort(all(ele));
    for (int i = 0; i < n; i++)
    ...
    for (int i = 1; i < n; i++)
    {
        auto it = lb(all(ele) , array<int,3> {ele[i][1] , -1, -1});
        if(it != ele.end()){
            int ind = (it-begin(ele));
            if(ind >= i ) continue;
            debug(ind)
            debug(i)
            int l = 0 , r = i-1 , mid , a = -1;
            while(l <= r){
                mid = (l+r)/2;
                if(st.sum(mid,i) <= ele[i][0]){
                    l = mid + 1;
                    a = mid;
                }
                else l = mid + 1;
            ...
            if(a != -1){
                if(ans[0] > sum - ele[a][0] - ele[i][0]){
                    ans[0] =  sum - ele[a][0] - ele[i][0];
                    ans[1] = ele[a][2];
                    ans[2]  = ele[i][2];
                }
            }
    ...
    array<ll , 3 > one{INF,-1,-1};
    
    cout << sum - ans[0] << nline;
    cout << ans[1] << " " << ans[2] << nline;
}   
int main(){
    FAST
    int t=1;
