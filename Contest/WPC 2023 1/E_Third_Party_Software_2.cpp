...
void solve(){
    ...
    for (int i = 0; i < n; i++)
    {
        int a , b;
        cin >>  a >>  b;
        ele.emplace_back(array<int,3> {a,b,i});
    }
    sort(all(ele));
    pr r{ele[0][0]  , ele[0][1]};
    for(auto x : ele){
        if(r.second >= x[1]) continue;
        if(r.second < x[0] - 1){
            cout << "NO\n";
            return;
        }
        r.second = max(r.second , x[1]);
    }
    debug(r)
    if(r.second != m || r.first != 1){
        cout << "NO\n";
        return;
    }
    ...
    for(auto x : ele){
        if(!maxi.count(x[0])){
            ...
            if(maxi[x[0]].first < x[1]) maxi[x[0]] = mk(x[1],x[2]);
        }
    }
    ele.clear();
    vi ans;
    ...
    for(auto x : maxi){
        ...
        if(x.first == reach + 1){
            ...
            ans.emplace_back(next.second+1);
            reach = next.first;
        }
        else{
            ans.emplace_back(next.second+1);
