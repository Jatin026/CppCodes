...
void solve(){
    ...
    cin >> n;
    vll val;
    vector<array<ll,3>> jedi;
    for (int i = 0; i < n; i++)
    {
        ll a, b, c;
        cin >> a>> b >> c;
        ll maxi  = max(a,b); maxi = max(maxi , c);
        val.pb(a+b+c-maxi);
        jedi.emplace_back(array<ll,3> {a,b,c});
    }
    sort(all(val));
    for (int i = 0; i < n; i++)
    {
        ll maxi = max(jedi[i][0] , jedi[i][1]); maxi = max(maxi , jedi[i][2]);
        ll check = jedi[i][0] + jedi[i][1] + jedi[i][2] ;
        auto it  = ub(all(val) , check-2);
        if(check - 2 < val.front()){
            cout << 0 << " ";
            continue;
        }
        --it;
        
        cout << (it-begin(val)) + ((*it) < (check - maxi) ? 1 :  0) << " ";
    }
