...
vector<int> pr;
void solve(){
    // nearest prime ki closest left ka ind and usse nearest non zero ind
    int n , e ;
    cin >> n >> e;
    vi v(n);
    for(auto &x : v) cin >> x;
    ...
    ll ans = 0;
    for(auto x : d){
        vector<pair<int,int>> ind; int cnt = 0;
        for (int i = 0; i < x.size(); i++)
        {
            if(x[i] == 1 || lp[x[i]] == x[i]){
                if(x[i] == 1){
                    if(!ind.empty()){
                        auto it = lb(all(ind),mk(cnt,0)) , it2 = lb(all(ind),mk(cnt-1,0));
                        ans += it - it2;
                        // if(it != it2) ans++;
                        int idx2 = (*it2).second , idx1 = it->second;
                        if(x[idx1] > 1 && x[idx2] == 1) ans++;
                        else if(idx1 == idx2 && x[idx1] > 1) ans++;
                         
                         
                    }
                    ind.emplace_back(cnt,i);
                }
                else{
                    if(!ind.empty()){
                        auto it = lb(all(ind),mk(cnt,0));
                        int idx = (*it).second;
                        ans += i - idx; 
                        if(x[idx] > 1) ans--;
