...
void solve(){
    ...
    // map<int,int> comp;
    // vector<vector<pair<int,int>>> ele;
    // for(auto x : v){
    //     int val = x;
    //     assert(x <= N);
    //     vector<pair<int,int>> fac{mk(1,val)};
    //     while(x > 1){
    //         if(fac.empty()) fac.emplace_back(lp[x],val);
    //         else{
    //             if(fac.back().first != lp[x]) fac.emplace_back(lp[x],val);
    //         }
    //         x /= lp[x];
    //     }
    //     for(auto x : fac){
    ...
    // for(auto x : ele){
    //     for(auto y : x){
    //         check[comp[y.first]].pb(y.second);
    //     }
    // }
    // for(auto &x : check) sort(rbegin(x),rend(x));
    // debug(check)
    // ll ans = 0;
    // for(auto x : check){
    //     int cnt = n;
    ...
    //     dp[i] = max(dp[i],frq[i]*1ll*i);
    //     for (int j = 2*i; j < N; j+=i)
    //     {
    //         dp[j] = max(dp[j] , (frq[i] - frq[j])*i + dp[j]);
    //     }
    //     // if(ans < dp[i]){
    //     //     cout << i << " ";
    ...
    for (int i = 1; i < N; i++)
    {
        ...
        for (int j = 2*i; j < N; j+=i)
        {
            dp[j] = max(dp[j] , dp[i] + frq[j]*1ll*(j-i));
        }
        ans = max(ans , dp[i]);
    }
