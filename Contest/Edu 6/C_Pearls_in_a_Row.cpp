...
void solve(){
    ...
    vpr range;
    for(auto &x : v) x = comp[x];
    int prev = 0;
    vi index(n , -1);
    // stac k<int> st;
    for(int i = 0; i < n ; i++){
        if(index[v[i]] != -1){
            for(int j = prev; j <= i; j++) index[v[j]] = -1;
            range.emplace_back(prev + 1,  i+1);
            prev = i + 1;
        }
        else{
            index[v[i]] = 0;
    ...
    if(range.size() == 0) cout << -1;
    else{
        range.back().second = n;
        cout << range.size() << nline;
        for(auto x : range) cout << x.first << " " << x.second << nline;
    }
}
int main(){
    FAST
