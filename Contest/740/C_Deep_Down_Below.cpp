...
void solve(){
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        range[i] = mk(maxi,maxi+k);
    }
    sort(all(range));
    debug(range)
    pr comp = range[0];
    ll ans = range[0].first;
    for (int i = 1; i < range.size(); i++)
    {
        if(comp.second < range[i].first){
            ans += range[i].first - comp.second;
            comp.second = range[i].second;
        }
        else{
            comp.second += range[i].second - range[i].first;
        }
    }
    cout << ans << nline;
}
