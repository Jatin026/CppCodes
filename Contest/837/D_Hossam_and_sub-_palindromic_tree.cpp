...
void pre(int v , vvi &adj , vi &vis , vi &depth , vi&par){
    vis[v] = 1;
    for(auto u : adj[v]){
        if(vis[u]) continue;
        par[u] = v;
        depth[u] = 1 + depth[v];
        pre(u,adj,vis,depth,par);
    }
}
string s;
void dfs(int v , vvi &adj , vi &vis , vvi &dp, vi&par){
    vis[v] = 1;
    for(auto u : adj[v]){
        if(vis[u]) continue;
        for (int i = par[u]; i != 0; i = par[i])
        {
            if(s[i-1] == s[u-1]){
...
void solve(){
    ...
    vi vis(n+1,0) , par(n+1,-1) , depth(n+1,0);
    pre(1,adj,vis,depth,par);
    int val  = maxe(all(depth));
    vi values;
    for (int i = 1; i <= n; i++)
    {
        if(depth[i] == val){
    ...
    for(auto x : values){
        if(cnt>3) break;
        fill(all(par),-1);fill(all(vis),0);fill(all(depth),0);
        pre(x,adj,vis,depth,par);
        vvi dp(n+1,vi(n+1,1));
        fill(all(vis),0);
        dfs(x,adj,vis,dp,par);
        cnt++;  
    }
}
int main(){
