...
void solve(){
    ...
    for (int i = 0; i < min(n,5000); i++)
    {
        ...
        {
            if(i == j) continue;
            if(sum[v[i]+v[j]][0] == min(i+1,j+1) || sum[v[i]+v[j]][1] == max(i+1,j+1) || sum[v[i]+v[j]][0] == max(i+1,j+1) || sum[v[i]+v[j]][1] == min(i+1,j+1) ) continue;
            if(sum[v[i]+v[j]][0] != -1){
                cout << "YES\n";
                cout << sum[v[i]+v[j]][0] <<" "<<sum[v[i]+v[j]][1] <<" "<<i+1<<" "<<j+1;
                return; 
            }
            sum[v[i]+v[j]][0] = min(i+1,j+1);
            sum[v[i]+v[j]][1] = max(j+1,i+1);
        }
