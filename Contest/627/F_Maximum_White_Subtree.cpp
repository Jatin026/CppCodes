...
void solve(){
    ...
    function<void(int,int,vi &)> dfs = [&](int v , int p ,vi &sum ){
    ...
    function<void(int,int,vi & , vi&)> calc = [&](int v , int p ,vi &sum , vi &ans){
    ...
    vi glob(n,-1e9);
    for (int root = 0; root < min(1000,n); root++)
    {
        vi sum(n,0),ans(n,-1e9);
        dfs(root,root,sum);
        calc(root,root,sum,ans);
        for (int i = 0; i < n; i++)
        {
            glob[i] = max(ans[i] , glob[i]);
        }
        debug(sum)
    }
    for(auto x : glob) cout << x << " ";
}
int main(){
    FAST
    int t=1;
