...
void solve(){
    ...
    for(int i = n/2 ; i > 1 ; i--){
        ...
        if(group[i].size()&1){
            vi temp;
            for(auto x : group[i]) if(x!=2*i) temp.pb(x);
            swap(temp,group[i]);
            vis[2*i] = 0;
        }
    }
    vi no;
    for(int i = 1 ; i <= 2*n ; i++) if(!vis[i]) no.pb(i);
    ...
    for(int i =  2; i <= n ; i++){
        if(!group[i].empty()){
            ...
            for(int j =0 ; j < group[i].size(); j+=2){
                ans.emplace_back(group[i][j] , group[i][j+1]);
            }
        }
    }
    cout << no.size()/2 << nline;
    for(auto x : ans){
        cout << x.first << " " << x.second << nline;
    }
    for (int i = 0; i < no.size(); i+=2)
    {
        cout << no[i] << " " << no[i+1] << nline;
    }
