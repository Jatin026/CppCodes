...
void solve(){
    ...
    vi subtree_size(n);
    function<int(int,int)> get_subtree_size = [&] (int node, int par = -1) {
    ...
    function<int(int,int)> get_centroid = [&] (int node, int par = -1) {
	for (int i : adj[node]) {
		if (i == par) continue;
    ...
    int node = get_centroid(0,-1);
    LCA lca(adj,node);
    auto dis = [&](int u , int v) -> int {
        return (lca.height[u] + lca.height[v] - 2*lca.height[lca.lca(u,v)]);
    };
    vvi up(n,vi(LOG));
    ...
    dfs(node,node);
    auto cen = [&](int u , int v ) -> int {
        int  t = lca.lca(u,v);
        int d1 = dis(t,u) , d2 = dis(t,v);
        int mid = (d1+d2+1)/2;
        if(d1 > d2){
            return go(u,mid);
        }
