...
bool pythagoras(vector<pair<ll,ll>> &v){
    vll dis;
    for (int i = 0; i < 3; i++)
    {
        for (int j = i+1; j < 3; j++)
        {
            dis.pb((v[i].first-v[j].first)*(v[i].first-v[j].first)+(v[i].second-v[j].second)*(v[i].second-v[j].second));
        }
    }
    sort(all(dis));
    debug(dis)
...
void solve(){
    ...
    debug(v)
    if(x.size() != 3 && y.size() != 3 && pythagoras(v)) cout << "NO\n";
    else cout << "YES\n";
