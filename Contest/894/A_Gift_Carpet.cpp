...
bool good(map<char,vector<int>> ind){
    // map<char,vector<int>> ind;
    // for(int i = 0 ; i < s.size() ; i++){
    //     ind[s[i]].pb(i);
    // }
    if(ind['v'].empty()) return 0;
    if(ind['i'].empty()) return 0;
    auto it = ub(all(ind['i']),(ind['v'].front()));
    if(it == ind['i'].end()) return 0;
    it = ub(all(ind['k']),(*it));
    if(it == ind['k'].end()) return 0;
    it = ub(all(ind['a']),(*it));
    if(it == ind['a'].end()) return 0;
    return 1;
}
void solve(){
    int n , m;
