...
const int ZERO = 1e5 + 7;
vector<Mint> dp(2*ZERO,0);
void solve(){
    int n;
    cin >> n;
    vi v(n);
    ...
    for (int i = 0; i < n-2 ; i++)
    {
        vector<Mint>  new_dp(2*ZERO,0);
        for(int x = -9e4-10 ;  x < 9e4+10 ; x++){
            new_dp[x+v[i+2]+ZERO] += dp[x+ZERO];
            if(x) new_dp[ZERO+v[i+2]-x] += dp[x+ZERO];
        }
        swap(new_dp,dp);
    }
    Mint ans = 0;
