...
void solve(){
    ...
    for(auto x : odd){
        o.pb({x.second,x.first});
    }
    for(auto x : even){
        e.pb({x.second,x.first});
    }
    sort(all(o)); reverse(all(o));
    sort(all(e)); reverse(all(e));
    if(o[0].second != e[0].second){
        cout << n/2 - e[0].first + n/2 -o[0].first;
    }
    else if(o.size() > 1  || e.size() > 1){
        int ans = 1e9;
        if(o.size() > 1){
            ans  =  n/2 - o[1].first + n/2 - e[0].first;
        }
        if(e.size() > 1){
            ans = min(ans , n/2 - e[1].first + n/2 - o[0].first);
        }
        cout << ans;
    }
    else{
        cout << min(n/2 + n/2 - o[0].first , n/2 + n/2 - e[0].first);
    }
}
int main(){
    FAST
