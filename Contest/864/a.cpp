...
int solution(){
    int n , t , c;
    cin >> n >> t >> c;
    vi input4(n), input5(n);
    vector<ld> dp(c+1,-1e9);
    vector<ld> val(n);
    for(int i = 0 ; i < n ; i++){
        val[i] = input4[i]*pow((1+input5[i]/100),t) - input4[i];
    }
    dp[0] = 0;
    for(int i =  0  ; i < n ; i++){
        vector<ld> new_dp = dp;
        for(int wt = 0 ; wt + input4[i] <= c ; wt++){
            new_dp[wt+input4[i]] = max(new_dp[wt+input4[i]],dp[wt]+val[i] );
        }
        swap(new_dp,dp);
    }
    int ans = 0;
