...
void solve(){
    __int128 x1, y1,r1 , x2,y2 ,r2;
    int a; cin >> a;
    x1 = a; cin>>a; y1 = a; cin>>a; r1 =a; cin >> a; x2 = a; cin >> a; y2 = a; cin >> a; r2 = a; 
    __int128 d = (x2-x1)*(x2-x1) + (y2-y1)*(y2-y1);
    cout << fixed << setprecision(20);
    if(d > (r1+r2)*(r1+r2)) cout << 0;
    else{
        if(d  <= (r1-r2)*(r1-r2)){
            cout << (lld)(PI)*min(r1,r2)*min(r2,r1);
        }
        else{
            lld dis = sqrtl(d);
            lld ans = r2*r2*acos((d  + r2*r2 - r1*r1 )/(2*dis*r2));
            
            ans += r1*r1*acos((dis*dis + r1*r1 - r2*r2 )/(2*dis*r1));
            ans -= (0.5)*sqrtl((r1+r2-dis)*(dis+r1-r2)*(dis+r2-r1)*(dis+r1+r2));
            cout << ans;
        }
    }
}
