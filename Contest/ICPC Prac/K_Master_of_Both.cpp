...
/*
    dp[i][j] = number of times such that ith character appeared before jth in the original sequence and with it the ordering changed.
*/
const int K = 26;
...
void solve(){
    ...
    //         if(i == j) continue;
    //         if(dp[i][j]){
    //             cout << char(i+'a') << "   " << char(j+'a')<< " "<<dp[i][j] << nline;
    //         }
    //     }
