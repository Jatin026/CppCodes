...
void solve(){
    ...
    for(auto &item : v){
        ...
        cin >> item.x >> item.y >> item.time >> item.val;
    }
    auto cmp = [&](info a , info b) -> bool {
        return (a.time < b.time);
    };
    sort(all(v),cmp);
    vll dp(n+1, -1e18);
    dp[0] = 0;
    auto possible = [&](info a , info b) -> bool {
        if((a.x-b.x)*(a.x - b.x) + (a.y - b.y)*(a.y - b.y) <= (b.time - a.time)*(b.time - a.time)) {
