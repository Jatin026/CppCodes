...
void solve(){
    ...
    for (int i = 1; i <= n; i++)
    {
        if(i+w-1<=n ) str[(pref[i+w-1]-pref[i-1])%9].insert(i);
        ...
        for (int i = 0; i < 10; i++)
        {
            for (int j = 0; j < 10; j++)
            {
                if(((i*rem)%9+j)%9==k){
                    ...
                    if(i!=j && str[i].size()>=1 && str[j].size()>=1){
                        auto it = str[i].begin();
                        auto it2 = str[j].begin();
                        ans.insert({*it , *(it2)});
                    }
