...
void solve(){
    string s;cin>>s;
    multiset<pair<char,int>> path;
    if(s[s.size()-1]>=s[0]){
        for (int i = 0; i < s.size(); i++)
        {
            if(s[s.size()-1]>=s[i] && s[i]>=s[0]){
                path.insert({s[i],i+1});
            }
        }
        cout<<s[s.size()-1]-s[0]<<" "<<path.size()<<"\n"; 
        for(auto x : path){
            cout<<x.second<<" ";
        }
        cout<<nline;
        ...
        cout<<abs(s[s.size()-1]-s[0])<<" "<<v.size()<<"\n"; 
        sort(all(v));
        reverse(all(v));
        cout<<1<<" ";
        for(auto x : v){
            if(x.second!=1 && x.second!=s.size()) cout<<x.second<<" ";
        }
        cout<<s.size()<<" "<<nline;
    }
