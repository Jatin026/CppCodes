...
void solve(){
    ...
    dfs(0,0);
    f = depth;
    ll val = (maxe(all(depth)));
    ll maxi = val*k;
    if(k<=c) cout << maxi << nline;
    else{
        int node = 0;
        for (int i = 0; i < n; i++)
        {
            if(depth[i] == val) node = i;
        }
        fill(all(depth),0);
        dfs(node,node);
        val = maxe(all(depth));
        ...
        vi path;
        for(int i = node2 ; i != node ; i = par[i]){
            path.pb(i);
        }
        path.pb(node);
        int len = path.size();
        ...
        cout <<  max(maxi,(len-1)*k - mini) << nline;
    }
}
int main(){
    FAST
