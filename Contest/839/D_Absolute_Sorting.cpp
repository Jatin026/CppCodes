...
void solve(){
    ...
    sum = (v[0] + temp2[n-1]+1)/2;
    if(good(v,sum)) {
        cout << sum << nline;
        return;
    }
    sum = (v[n-1] + temp2[0] + 1)/2;
    if(good(v,sum)) {
        cout << sum << nline;
        return;
    }
