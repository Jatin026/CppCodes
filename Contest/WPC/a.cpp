...
vector<int> largestPrime(N,1);
static int c = 0;
void pre(){
    isPrime[1]=0;
    if(!c){
        for(int i = 2 ; i< N ; i++){
            if(isPrime[i]) largestPrime[i]=i;
            for(int j = 2*i ; j<N ; j+=i){
                isPrime[j]=0;
                if(isPrime[i]) largestPrime[j]=i;
            }
        }
    }
    c++;
}
int maximumOR(vector<int> &a){
    vector<int> vis(22,0);
    for(int i =  0  ; i < a.size() ; i++){
        for(int bit = 0 ; bit < 22 ; bit++){
            if((1<<bit)&a[i]) vis[bit]=1;
