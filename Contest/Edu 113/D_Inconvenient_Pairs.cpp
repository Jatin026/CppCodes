...
void solve(){
    ...
    vi x(n) , y(m);
    set<int> bx , by;
    for(auto&xx : x) {cin >> xx; bx.insert(xx);}for(auto&xx : y) {cin >> xx; by.insert(xx);}
    map<int,vi> px , py; 
    vpr points(k);
    for(auto &x : points){
        cin >> x.first >> x.second;
        if(bx.count(x.first) && by.count(x.second)) continue;
        if(bx.count(x.first)) {px[x.first].pb(x.second) ;}
        if(by.count(x.second)) {py[x.second].pb(x.first) ;}
    ...
    for (int i = 0; i < n-1; i++)
    {
        auto l = ub(all(points) , pair<int,int> {x[i],1e9});
        auto r = ub(all(points) , pair<int,int>{x[i+1]-1,1e9});
        ll len = (r-l);
        ans += (len*(len-1))/2;
        debug(len)
    }
    ...
    debug(points)
    for(auto &x  : points) swap(x.first,x.second);
    sort(all(points));
    for (int i = 0; i < m-1; i++)
    {
        auto l = ub(all(points) , pair<int,int> {y[i],1e9});
        auto r = ub(all(points) , pair<int,int> {y[i+1]-1,1e9});
        ll len = (r-l);
        ans += (len*(len-1))/2;
    }
    for(auto sy : py){
        map<pr,ll> temp;
        for(auto xy : sy.second){
            auto it = (lb(all(x),xy));
            int b = (*it) , a = (*(--it));
            temp[mk(a,b)]++;
        }
    ...
    // for(auto x : px) ans -= (x.second*(x.second-1))/2;
    // for(auto x : py) ans -= (x.second*(x.second-1))/2;
    cout << ans << nline;
}
int main(){
    FAST
