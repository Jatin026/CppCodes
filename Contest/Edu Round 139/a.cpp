...
void solve(){
    ...
    for (int i = 0; i < m; i++)
    {
        ll a,b,w;
        cin >> a >> b >> w;
        if(edge.count(make_pair(a,b))){
            edge[make_pair(a,b)] = min(edge[make_pair(a,b)],w);
        }
        else edge[make_pair(a,b)] = w;
    }
    for(auto x : edge){
        adj[x.first.first].pb(make_pair(x.first.second,x.second));
        adj[x.first.second].pb(make_pair(x.first.first,x.second));
    }
    dijkstra(1,d,p);
    int ans = 0;
    for (int i = 0; i < k; i++)
