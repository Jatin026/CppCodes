...
bool valid(int x , int y , int n){
    if(x>-1 && x<2 && y>-1 && y < n && v[x][y] == 'B' && !grid[x][y]) return 1;
    else return 0; 
}
void dfs(int x , int y,int n){
    ...
    debug(x)debug(y)
    grid[x][y] = 1;
    if(valid(x+1,y,n)) dfs(x+1,y,n);
    else {
        if(valid(x-1,y,n)) dfs(x-1,y,n);
        else{
            if(valid(x,y+1,n)) dfs(x,y+1,n);
            else {
                debug(x)
                debug(y+1)
                return;
...
// run dfs from (1,1 or 2,1) and check if it visits all the black cell
void solve(){
    int n;
    cin >> n;
    v.resize(2);
    grid.assign(2,vi(n,0));
    for(auto &x : v){
        cin >> x;
    }
    debug(v)
