...
void solve(){
    ...
    for(int i = 1; i <  n; i++){
        ...
        cin >> a;
        --a;
        adj[a].emplace_back(i,i-1);
        adj[i].emplace_back(a,i-1);
    }
    bool flag = false;
    for(int i = 1; i < n; i++){
        if(adj[i].size() > 1) flag = 1;
    }
    vi col(n-1,-1);
    vi cnt(2,0);
    int LIM = 2;
    if(!flag){
        ...
        while(!q.empty()){
            ...
            for(auto u : adj[v]){
                ...
                par[u.first] = v;
                q.push(u.first);
                if(adj[u.first].size() == 2 && adj[v].size() == 2 && v) {
                    // cnt[d[v]%2]++;
                    flag = 1;
                    break;
                }
        ...
        if(flag) {
                ...
                if(adj[i].size() == 2){
                    ...
                    for(int j = i; j != 0; j = par[j]){
                        for(auto x : adj[j]){
                            if(x.first == par[j]){
                                if(col[x.second] != -1) {
                                    if(col[x.second] != move) {LIM = 3 ; break;}
                                }
                                col[x.second] = move;
                                move ^= 1;
                            }
            ...
            if(LIM == 3){
                ...
                while(!q.empty()){
                    ...
                    for(auto x : adj[v]){
                        if(x.first == par[v]){
                            if(col[x.second] != -1) move = (1^col[x.second]);
                        }
                    }
                    for(auto u : adj[v]){
                        if(vis[u.first]) continue;
