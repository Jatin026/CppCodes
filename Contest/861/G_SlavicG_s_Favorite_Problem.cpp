...
void solve(){
    ...
    for (int i = 0; i < n-1; i++)
    {
        ...
        cin >> u >> v >> w;
        --u;--v;
        adj[v].emplace_back(array<int,2> {u,w}); adj[u].emplace_back(array<int,2> {v,w});
    }
    set<int> s;
    function<void(int,int,int)> pre = [&](int v , int p , int x){
        for(auto [u,w] : adj[v]){
            if(u == p) continue;
            s.insert(x^w);
            pre(u,v,x^w);
    ...
    pre(b,b,0);
    bool flag = false;
    function<void(int,int,int)> ans = [&](int v , int p , int x){
        if(flag) return;
        if(s.count(x)){
            flag = 1;
            return;
