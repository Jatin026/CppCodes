...
void solve(){
    ...
    ll ans = 1e18;
    ll cos = 1e12;
    ans = min(ans , (cos+1)*pref[n]);
    ans = min(ans , (cos+1)*(n-pref[n]));
    if(is_sorted(all(s))) ans = 0;
    vi next(n);
    int ind = -1;
    for (int i = n-1; i > -1; i--)
    {
    ...
    for (int i = 0; i < n; i++)
    {
        // considering the ith index to be the starting 1 
        ll rem = pref[i];
        //considering no swap
        rem  += (n-i - (pref[n] - pref[i]));
        ans = min(ans , (cos+1)*rem);
        rem = 0;
        // considering a single swap
        if(i+1<n && next[i+1] != -1){
            int r = next[i+1];
            rem += (r-i-1);
            rem += pref[i];
            rem += (n - r - 1 -(pref[n] - pref[r+1]));
            ans = min(ans , cos + rem*(cos+1));
        }
    }
    cout << ans << nline;
