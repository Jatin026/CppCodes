...
map<array<ll,3>,bool> dp;
bool rec(ll i  , ll wt , ll s , ll n,ll k){
    if(s<0) return 0;
    if(i == n) return (s == 0);
    if(dp.count({i,wt,s})) return dp[{i,wt,s}];
    if(wt>s) return dp[{i,wt,s}] = rec(i+1,wt,s,n,k);
    else{
        return dp[{i,wt,s}] = (rec(i+1,wt*k,s-wt,n,k)|rec(i+1,wt*k,s+wt,n,k)|rec(i+1,wt*k,s,n,k));
    }
}
