...
void solve(){
    ...
    for (int i = 0; i < n-1; i++)
    {
        ...
        cin >> a >> b;
        --a; --b;
        bool flag = (val[a]|val[b]);
        adj[a].pb(b);adj[b].pb(a);
        ans[a] |= flag;ans[b] |= flag;
    }
    for (int i = 0; i < n; i++)
    {
        if(ans[i] && !val[i]) c[i] = 1;
    }
    ...
    function<void(int,int)> calc = [&](int v , int p) {
        if(ms.ub(1) != ms.end()) ans[v] = 1;
        else{
            for(auto u : adj[v]){
                if(sum[u] > 1 && c[u]) ans[v] = 1;
            }
        }
        for(auto u : adj[v]){
            if(u == p) continue;
            if(val[v]) ms.erase(ms.find(sum[v]));
            sum[v] -= sum[u];
            if(val[v]) ms.insert((sum[v]));
            if(val[u]) ms.erase(ms.find(sum[u]));
            sum[u] += sum[v];
            if(val[u]) ms.insert((sum[u]));
            calc(u,v);
            if(val[u]) ms.erase(ms.find(sum[u]));
            sum[u] -= sum[v];
            if(val[u]) ms.insert((sum[u]));
    ...
    function<void(int,int)> bhar_do = [&](int v , int p){
        ans[v] = 1;
        for(auto u : adj[v]){
            if(u == p) continue;
            bhar_do(u,v);
    ...
    for (int i = 0; i < min((int)kaalu.size(),5); i++)
    {
        for (int j = 0; j < min((int)kaalu.size(),5); j++)
        {
            if(i == j) continue;
            vi vis(n);
            dfs(kaalu[i] , kaalu[i]);
            for(auto u : adj[par[kaalu[j]]]){
                if(u == kaalu[j] || u == par[par[kaalu[j]]]) continue;
                bhar_do(u,par[kaalu[j]]);
            }
            // vis[kaalu[i]] = 1;
            // for(int p = kaalu[j] ; p != kaalu[i] ; p = par[p]){
            //     vis[p] = 1;
            // }
            // debug(vis)
            // for(int p = kaalu[j] ; p != kaalu[i] ; p = par[p]){
            //     for(auto u : adj[p]){
            //         if(!vis[u]) bhar_do(u,p);
            //     }
            // }
        }
    }
