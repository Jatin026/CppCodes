...
void solve(){
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        {
            ll dif = v[j] - v[i];
            for (ll factor = 1; factor*factor <= dif; factor++)
            {
                if(dif%factor == 0){
                     
                    ll b  = (dif/factor + factor)/2;
                    possible[b*b-v[j]] = 1;
                }
            }
    ...
    for(auto x : possible){
        ...
        for (int i = 0; i < n; i++)
        {
            ll val = sqrtl(v[i]+x.ff);
            if(val*val == v[i]+x.ff) cnt++;
