...
void solve(){
    ...
    vector<pair<int,char>> fav;
    for (int i = 1; i < 27; i++)
    {
        fav.emplace_back(cnt[i],char(i+96));
    }
    sort(fav.rbegin(),fav.rend());
    
    vector<bool> good(27,0);
    vector<pair<int,string>> ans;
 
    for (int j = 1; j < 27; j++)
    {
        string t = "";
        good[fav[j-1].second-96] = 1;
        if(n%j == 0){
            ...
            for (int k = 0; k < 27; k++)
            {
                if(!good[k]) continue;
                qnt[k] = n/j;
                for (int i = 0; i < (n/j) - cnt[k]; i++)
                {
                    available.emplace_back(char(k+96));
                }
            }
    ...
    cout <<ans[0].first << nline << ans[0].second<<nline;
}
int main(){
    FAST
    int t=1;
