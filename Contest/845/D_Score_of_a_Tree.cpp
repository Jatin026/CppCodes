...
int dfs(int v , vvi &adj , vi &vis , vi &height, vi &depth){
    vis[v] = 1;
    int maxi = depth[v]+1;
    for(auto u : adj[v]){
        if(vis[u]) continue;
        maxi = max(maxi , dfs(u,adj,vis,height,depth));
    }
    height[v] = maxi - depth[v];
    return maxi;
}
void solve(){
    int n;
    cin >> n;
    vvi adj(n+1);
    vi vis(n+1,0) , depth(n+1,1) , par(n+1,0), height(n+1,1);
    for (int i = 0; i < n-1; i++)
    {
        int a , b ;
        cin >> a >> b;
    ...
    fill(all(vis),0);
    dfs(1,adj,vis,height,depth);
    Mint ans  = 0 , k = BinExpItr(2,n-1);
    debug(height)
    for (int i = 1; i <= n; i++)
    {
        ans += k*height[i];
