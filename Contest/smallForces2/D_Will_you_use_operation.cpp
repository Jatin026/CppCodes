...
void solve(){
    ...
    for (int i = 0; i < n; i++)
    {
        if(s[i] == '1') one++;
        if(range.empty()){
            range.emplace_back(s[i]- '0' , 1);
        }
        else{
            if(range.back().first == s[i] - '0') range.back().second++;
            else range.emplace_back(s[i]- '0' , 1);
        }
    }
    n = range.size();
    vi suff(n+1,0);
    for (int i = n-1; i > -1; i--)
    {
        if(!range[i].first) suff[i] = max(suff[i+1],range[i].second);
    }
    debug(range)
    for (int i = 0; i < n; i++)
    {
        if(range[i].first){
            ...
            ans = max(ans , one - range[i].second + max(suff[i+1],tot));
        }
