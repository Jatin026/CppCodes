...
void solve(){
    ...
    auto dis = [&](array<int,4> &a) -> ll {
        ll x = a[2]-a[0] , y = a[3]-a[1];
        return (x*x+y*y);
    };
    auto cmp = [&](array<int,4> &a , array<int,4> &b) -> bool {
        if(dis(a) == dis(b)) return (a[0] < b[0]);
        return (dis(a) < dis(b));
    };
    sort(all(c),cmp);
    auto check = [&](array<int,4> &a, pr  &p) -> bool {
        // if(p.first == a[0] && p.second <= a[3] && p.second >= a[1]) return 1;
        // if(p.first == a[2] && p.second <= a[3] && p.second >= a[1]) return 1;
        // if(p.second == a[1] && p.first <= a[2] && p.first >= a[0]) return 1;
        // if(p.second == a[3] && p.first <= a[2] && p.first >= a[0]) return 1; 
        if(p.first >= a[0] && p.first <= a[2] && p.second >= a[1] && p.second <= a[3]) return 1;
        return 0;
    };
    vpr f;
    for (int i = 0; i < min(1000,n); i++)
    {
        f.push_back(mk(c[i][0],c[i][1]));
        f.push_back(mk(c[i][2],c[i][3]));
        f.push_back(mk(c[i][0],c[i][3]));
        f.push_back(mk(c[i][2],c[i][1]));
        // f[mk(c[0],c[1])]++; f[mk(c[2],c[3])]++;
        // f[mk(c[0],c[3])]++; f[mk(c[2],c[1])]++;
    }
    reverse(all(c));
    for (int i = 0; i < min(1000,n); i++)
    {
