...
void make_set(int v , vi &parent , vll &size) {
    parent[v] = v;
    size[v] = 1;
}
int find_set(int v, vi &parent , vll &size) {
    if (v == parent[v])
        return v;
...
int check  ;
void union_sets(int a, int b, vi &parent , vll &size) {
...
void solve(){
    ...
    for (int i = 0; i < n; i++)
    {
        make_set(i+1,parent,size);
    }
    make_set(0,parent,size);
    for (int j = 0; j < 32; j++)
    {
        bool f = false;
        int ind = 0;
        for (int i = 0; i < n; i++)
        {
            if(((1ll<<j)&(v[i])) == 0) continue;
            if(find_set(ind,parent,size) == find_set(i+1,parent,size)) continue;
            // union_sets(0,i+1,parent,size);
            if(f){
                union_sets(ind,i+1,parent,size);
                debug(ind) debug(i+1)
                ans+=(1<<j);
            }
            else{
