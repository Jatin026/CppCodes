...
void solve(){
    ...
    while(!pq.empty()){
        auto vv  = *(pq.begin());
        ll w = vv.first , v = vv.second;
        pq.erase(pq.find(vv));
        for(auto u : adj[v]){
            if(dis[u[0]] > u[1] + dis[v]){
                if(pq.count(mk(dis[u[0]] , u[0] )))
                    pq.erase(mk(dis[u[0]] , u[0] ));
                dis[u[0]] = dis[v] + u[1];
                pq.insert(mk(dis[u[0]] , u[0] ));
            }
    ...
    /*
        ...
        u(0) -> u
        u(1) -> wt of edge
        w v tak pahuchne shortest dist
    ...
    */
    ...
    map<ll,vector<int>> nodes;
    for (int i = 0; i < n; i++)
    {
        if(dis[i] < INF && (dis[i]) && dis[i] <= dis[0]){
            nodes[dis[i]].pb(i);
        }
    }
    string ans (n , '1');
    ...
    ans[n-1] = '0';
    ll maxi = 0;
    if(!nodes.empty()) maxi = (*(nodes.rbegin())).first;
    else{
        cout << 0  << " " << 0 << nline;
        // cout << ans << nline;
        return;
    }
    cout << maxi  << " " << (int)nodes.size() << nline;
    for(auto it = nodes.begin() ;   it != nodes.end(); it++){
        // if(ans[0] == '0') break;
        cout << ans << " " << ((*it).first) - more << nline;
        for(auto x : (*it).second) ans[x] = '0';
        more = (*it).first;
    } 
}
int main(){
