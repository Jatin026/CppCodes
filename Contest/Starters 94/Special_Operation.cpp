...
void solve(){
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < m; j++)
        {
            val.emplace_back(array<ll,3> {tot - row[i] - col[j] + (ll)(n*m - n - m)*v[i][j],i,j});
        }
    }
    sort(rbegin(val),rend(val));
    ll ans = 0;
    for (int i = 0; i < min(100,n*m); i++)
    {
        ll curr = 0;
        for (int x = 0; x < n; x++)
        {
            for (int y = 0; y < m; y++)
            {
                if(x != val[i][1] && y != val[i][2]){
                    curr += (v[val[i][1]][val[i][2]] ^ v[x][y]);
                }
            }
        }
        ans = max(ans , curr);
    ...
    if(n <= 10 ||  m <= 10){
        for (int i = 0; i < min((int)500,n*m); i++)
        {
            ll curr = 0;
            for (int x = 0; x < n; x++)
            {
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < m; j++)
        {
            val.emplace_back(array<ll,3> {v[i][j],i,j});
        }
    }
    sort(rbegin(val),rend(val));
    for (int i = 0; i < min(100,n*m); i++)
