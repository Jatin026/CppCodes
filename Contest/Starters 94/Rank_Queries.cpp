...
void solve(){
    ...
    FenwickTree inc(n+1) , dec(n+1);
    for(auto x : v) dec.add(x,1);
    vector<array<int,2>> pre(n);
    for (int i = 0; i < n; i++)
    {
        pre[i][0] = inc.sum(v[i]+1,n);
        pre[i][1] = dec.sum(v[i]+1,n);
        inc.add(v[i],1);dec.add(v[i],-1);
    }
    auto check = [&](ll x  , ll cnt , ll ind) -> bool {
        ll l = pre[ind][0] , r  = pre[ind][1];
        l -= x; r -= x;
        if(max(l,0*1ll) + max(r,0*1ll) <= cnt) return 1;
        else return 0;
    };
    for (int i = 0; i < q; i++)
    {
