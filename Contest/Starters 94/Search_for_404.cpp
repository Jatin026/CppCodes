...
void solve(){
    ...
    {
        if(s[i] != '4'){
            ll l4 = cnt[i][1] , ls = cnt[i][2] , r4 = cnt[n][1] - cnt[i+1][1] , rs = cnt[n][2] - cnt[i+1][2];
            debug(l4) debug(ls)  debug(rs) debug(r4)
            if(ls == 0 && rs  == 0){
                ans += (l4*r4);
            }
            else if(ls == 0){
                ...
                Mint a = (2*l4 + ls) , b = (2*r4 + rs); 
                a *= BinExpItr(2,ls-1); b *= BinExpItr(2,rs-1);
                ans += (a*b);
            }
        }
