...
void solve(){
    ...
    auto sf = atcoder::suffix_array(s);
    auto lcp = atcoder::lcp_array(s,sf);
    pair<int,int> ans{1e9,1e9};
    if(n - sf[0] > lcp[0]) ans = {lcp[0],sf[0]};
    if(n -2 >= 0 && n - sf[n-1] > lcp[n-2]) ans = min(ans,{lcp[n-2],sf[n-1]});
    for(int i = 0; i < n -2; i++){
        if(n - sf[i+1] > max(lcp[i],lcp[i+1]))
            ans = min(ans ,{max(lcp[i],lcp[i+1]) , sf[i+1]});
    }
    if(ans.first > n) cout << s;
    else {
        cout << s.substr(ans.second, ans.first+1);
    }
}
int main(){
    FAST
