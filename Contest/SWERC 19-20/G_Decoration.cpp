...
void solve(){
    ...
    for(auto x : prob){
        ...
        vi same;
            ...
            while(j != -1){
                ...
                if(j == -1) break;
                if(vis[j]){
                    if(vis[j] == time) prob.pb(j);
                    break;
                }
                vis[j] = time;
            }
            time++;
        for(auto y : same) lim[y] = same.size();
    }
    int barr = time;
    for(int i = 0 ; i < n ; i++){
        if(!vis[i]){
            ...
            while(!same.empty()){
                lim[same.back()] = min(++touch,lim[same.back()]);
                same.pop_back();
            }
    ...
    for(int i = 1; i < K ; i++){
        for(int j = 0; j < N; j++){
            if(sp[j][i-1] != -1){
                ...
                sum[j][i] = sum[sp[j][i-1]][i-1] + sum[j][i-1];
            }
        }
    }
    auto jump = [&](int splash, int v) -> ll {
        ll ans = 0;
        for(int i = 0 ; i < K; i++){
            if((1<<i)&splash){
                ans += sum[v][i];
    ...
    pair<ll,ll> ans{1e17,1e12};
    --k;
    for(int i = 0; i < n; i++){
        if(jump(k,i) != -1 && (k+1) < lim[i]) ans = min(ans , {jump(k,i) , i});
    }
    if(ans.first > 1e16) cout << -1 << nline;
    else{
        map<int,int> f;
        ll sum = 0;
        cout << ans.second  << " ";
