...
void solve(){
    ...
    for(auto x : col){
        ...
        debug(pref)
        int mid = (n + x[len-1]+1)/2;
        ans = min(ans , max(g , max(n-mid , mid - x[len-1] - 1) ));
        mid = (n + x[len-1])/2;
        ans = min(ans , max(g , max(n-mid , mid - x[len-1] - 1) ));
        int curr  = (n - x[len-1]);
        for(int i = len - 1 ;  i >  0 ; i--){
            ...
            mid = (x[i] + x[i-1])/2;
            ans = min(ans ,  max(max(pref[i-1],curr), max(mid - x[i-1] - 1 , x[i] - mid - 1)  ));
            mid = (x[i] + x[i-1] + 1)/2;
            ans = min(ans ,  max(max(pref[i-1],curr), max(mid - x[i-1] - 1 , x[i] - mid - 1)  ));
            curr = max(curr , x[i] - x[i-1] - 1);
        }
        mid = (1 + x[0])/2;
        ans = min(ans , max(curr , max(mid -  1 , x[0] - mid - 1)));
        mid = (2 + x[0])/2;
        ans = min(ans , max(curr , max(mid -  1 , x[0] - mid - 1)));
        g = max(g , n - x[len-1]);
        ans = min(ans , g);
