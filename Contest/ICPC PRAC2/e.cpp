...
int gauss (vector < bitset<LOG+1> > a, int n, int m, bitset<LOG+1>ans) {
    ...
    for (int i=0; i<m; ++i){
        if (where[i] != -1){
            assert(int(a[where[i]][i]) > 0);
            ans[i] = a[where[i]][m] / a[where[i]][i];
            debug(int(a[where[i]][m]))
        }
    }
    for (int i=0; i<n; ++i) {
        Mint sum = 0;
...
bool possible(int a , int b){
    ...
    for (int bit = LOG - 1; bit > -1 ; bit--)
    {
        ...
        else if(!(a & (1<<bit)) && (b&(1<<bit))) return 0;
    
    }
    return gauss(mat,int(mat.size()),LOG,ans);
    // return (rankOfMatrix(mat) >= rankOfMatrix(aug_mat));
}
