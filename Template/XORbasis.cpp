...
void insertVector(int mask , int &sz , vi &basis) {
	for (int i = 0; i < d; i++) {
...
void solve(){
    ...
    for (int i = 0; i < n; i++)
    {
        pre[i+1] = pre[i];
        sz[i+1] = sz[i];
        insertVector(v[i],sz[i+1],pre[i+1]);
    }
    while(q--){
        int i , x;
        cin >> i >> x;
        if(check(x,pre[i])){
            cout << BinExpItr(2,i-sz[i]) << nline;
        }
        else cout << 0 << nline;
    }
}
