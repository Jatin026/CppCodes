...
void solve(){
    ...
    sort(all(c)); 
    c.erase(unique(all(c)),c.end()); // Set of all unique integers given in the input.
    for (int i = 0; i < c.size(); i++)
    {
        m[c[i]]=hash(c[i]); // Designating a Random value to each one of them.
    }
    map<ll,int> visA,visB;
    debug(m)
   
    for (int i = 0; i < n; i++)
    {
        if(!visA.count(a[i]))
        prefA[i+1]=(prefA[i]+m[a[i]]);
        else prefA[i+1]=prefA[i];
        visA[a[i]]++; //Distinct XOR array , basically XOR will only be equal if the set of numbers used are same. Low chance of Collision due to random integers.
         prefA[i+1]%=N;
        if(!visB.count(b[i])) prefB[i+1]=(prefB[i]+m[b[i]]);
