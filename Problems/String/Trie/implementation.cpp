...
void solve(){
    ...
    for(auto x : words){
        int curr = 0;
        for(int i = 0 , v ; i < x.size()  ; i++  ){
            v = x[i] - 'a';
            if(trie[curr].nodes[v] == -1){
                trie[curr].nodes[v] = trie.size();
                trie.emplace_back();
            }
            if(i == x.size() - 1 ) trie[curr].terminal = 1;
            curr = trie[curr].nodes[v];
