...
vector<ll> v;
const int N =  5005;
pair<ll,ll> NullValue= {-1,-1};
pair<ll,ll> dp[N][N];
int n;
pair<ll,ll> rec(int l , int r  , int p){
    if(l-r==1) return {0,0};
    if(l>r ) return {-1e15,-1e15};
    if(dp[l][r]!=NullValue){
        return dp[l][r];
    }
    if(p){
        pair<ll,ll> p1 = rec(l+1,r,!p);
        pair<ll,ll> p2 = rec(l,r-1,!p);
        if(p1.first+v[l] > p2.first + v[r]){
            return dp[l][r]={p1.first+v[l],p1.second};
        }
        else{
            return dp[l][r]={p2.first+v[r],p2.second};
        }
    }
    else{
        pair<ll,ll> p1 = rec(l+1,r,!p);
        pair<ll,ll> p2 = rec(l,r-1,!p);
        if(p1.second+v[l] > p2.second + v[r]){
            return dp[l][r]={p1.first,p1.second + v[l]};
        }
        else{
            return dp[l][r]={p2.first,p2.second + v[r]};
        }
    }
}
