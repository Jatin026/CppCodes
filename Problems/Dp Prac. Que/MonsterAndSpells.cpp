...
void solve(){
    ...
    for (int i = 0; i < n; i++)
    {
        if(p.second>seg[i].first){
            p.first = min(p.first,seg[i].first);
            p.second = max(p.second,seg[i].second);
        }
        else{
            fin.pb(p);
            p=seg[i];
    ...
    if(fin.size() == 0) fin.pb(p);
    if(fin.back()!=p) fin.pb(p);
    ll ans = 0;
    for (int i = 0; i < fin.size(); i++)
    {
        ll len = fin[i].second - fin[i].first;
        ans+=(len*(len+1))/2;
    }
    cout << ans << nline;
    debug(fin)
