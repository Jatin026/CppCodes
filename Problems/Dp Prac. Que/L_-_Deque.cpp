...
const ll INF = 1e14;
ll dp[2][N][N];
ll rec(int i,int j , int move , int n , vi &v){
    if(j<i)  return 0;
    if(move){
        if(dp[move][i][j] != -INF) return dp[move][i][j];
    }
    else{
        if(dp[move][i][j] != INF) return dp[move][i][j];
    }
    if(move){
        ll ans = INF;
        ans = min(ans,rec(i,j-1,(1^move),n,v) - v[j-1]);
        ans = min(ans, rec(i+1,j,(1^move),n,v) - v[i-1]);
        debug(ans)
        return dp[move][i][j] = ans;  
    } 
    else{
        ll ans = -INF;
        ans = max(ans,rec(i,j-1,(1^move),n,v) + v[j-1]);
        ans = max(ans, rec(i+1,j,(1^move),n,v) + v[i-1]);
        debug(ans)
        return dp[move][i][j] = ans; 
    }
...
void solve(){
    ...
    // dp[1][0][n] = 0 ; dp[0][0][n] = 0;
    // for (int i = 0; i <= n; i++)
    // {
    //     for (int j = n; j > i; j--)
    //     {
    //         if((i-1 + (n-j))%2 == 1 ){
    //             dp[1][i][j-1] = min(dp[1][i][j-1], dp[0][i][j] - v[j-1]);
    //             dp[1][i+1][j] = min(dp[1][i+1][j], dp[0][i][j] - v[i-1]);
    //             dp[0][i][j-1] = dp[0][i+1][j] = dp[0][i][j];
    //         }
    //         else{
    //             dp[0][i][j-1] = max(dp[1][i][j-1], dp[1][i][j] + v[j-1]);
    //             dp[0][i+1][j] = max(dp[1][i+1][j], dp[1][i][j] + v[i-1]);
