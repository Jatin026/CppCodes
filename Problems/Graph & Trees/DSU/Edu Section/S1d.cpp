...
void solve(){
    ...
    {
        if(querry[i].first=="cut"){
            union_sets(querry[i].second.first,querry[i].second.second);
            debug(querry[i].second.second)
        }
        else{
            ans.pb(find_set(querry[i].second.first)==find_set(querry[i].second.second));
        }
    }
    reverse(all(ans));
