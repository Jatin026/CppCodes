...
void solve(){
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        {
            ans[i] = min(ans[i] , abs(j-i) + abs(v[i]-v[j]));
        }    
        for (int j = i-1; j > max(i-10,0); j--)
        {  
            ans[i] = min(ans[i] , abs(i-j) + abs(v[i]-v[j]));
        }
        for (int j = v[i]+1; j < min(n+1,v[i]+10); j++)
        {
            ans[i] = min(ans[i] , abs(j-v[i]) + abs(ind[v[i]]-ind[j]));     
        }
        for (int j = v[i]-1; j > max(v[i]-10,0); j--)
        {
            ans[i] = min(ans[i] , abs(v[i]-j) + abs(ind[v[i]]-ind[j]));
        }
    }
    map<int,vi> sub , add ;
    vi equal;
    ...
    for (int i = 0; i < n; i++)
    {
        // auto it  =  ub(all(sub[v[i]-i]),i);
        // if(it != sub[v[i]-i].end()) ans[i] = min(ans[i] ,  2*abs((*it)-i));
        // it = lb(all(sub[v[i]-i]),i);
        for(auto x : sub[v[i]-i]){
            if(x == i) continue;
            ans[i] = min(ans[i],2*abs(x-i));
        }
        for(auto x : add[v[i]+i]){
            if(x == i) continue;
            ans[i] = min(ans[i],2*abs(x-i));
        }
        // if(it != sub[v[i]-i].begin()) ans[i] = min(ans[i] , 2*abs((*(--it))-i));
        // it = ub(all(add[v[i]+i]),i);
        // if(it != add[v[i]+i].end()) ans[i] = min(ans[i] , 2*abs((*it)-i));
        // it = lb(all(add[v[i]+i]),i);
        // if(it != add[v[i]+i].begin()) ans[i] = min(ans[i] , 2*abs((*(--it))-i));
