...
void solve(){
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        // vector<set<int>> prime(10);
        while(temp!=1){
            fac[lp[temp]]++;
            // prime[lp[temp]%10].insert(lp[temp]);
            temp/=lp[temp];
        }
        if(fac.size() == 1) continue;
            ...
            if(v[i]&1){
                temp = v[i];
                int val = (*(fac.begin())).first;
                while(temp%val == 0) temp/=val;
                ans[i].first = val;
                ans[i].second = temp;
                // for (int j = 0; j < 10; j++)
                // {
                //     if(prime[i].size()>1){
                //         auto it = prime[j].begin();
                //         ans[i].first = (*it);
                //         ++it;
                //         ans[i].second =(*it);
                //         flag = 1;
                ...
                //         {
                //             if((l+k)%10 !=5 && prime[l].size()>0 && prime[k].size()>0){
                //                 ans[i].first = *(prime[l].begin());
                //                 ans[i].second = *(prime[k].begin());
                //                 flag = 1;
                //                 break;
                //             } 
                //         }
