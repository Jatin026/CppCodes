...
void solve(){
    ...
    for(auto x : sources){
        ...
        while(!q.empty()){
            ...
            q.pop();
            for(auto y : dir){
                if(valid(s.ff+y.ff,s.ss+y.ss) && !vis[make_pair(s.ff+y.ff,s.ss+y.ss)]){
                    q.push(make_pair(s.ff+y.ff,s.ss+y.ss));
                    dis[make_pair(s.ff+y.ff,s.ss+y.ss)] = 1 + dis[s];
                    vis[make_pair(s.ff+y.ff,s.ss+y.ss)] = 1;
                }
            }
        }
        for (int i = 0; i < n; i++)
        {
            for (int j = 0; j < m; j++)
            {
                if(dis[make_pair(i,j)] <= v[x.ff][x.ss] - '0' && (v[i][j] < '0' || v[i][j] > '9')){
                    v[i][j] = '.';
                }
            }    
        }
