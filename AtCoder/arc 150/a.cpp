...
void solve(){
    ...
    {
        vi fac;
        for (int i = 1; i*i <= (b+j); i++)
        {
            if((b+j)%i==0){
                fac.pb(i);fac.pb((b+j)/i); 
            }
        }   
        sort(all(fac));
        auto it = lb(all(fac),a);
        ans=min(ans,(ll)(*it-a)+j);
        
    }
    

    
    for (int j = 0; j <= min((ll)10000,abs(b-a)); j++)
    {
        debug(ans)
        ans=min(ans,(ll)((b+a-1+j)/(a+j))*(a+j)-b+j);
        
    }
 
    ans=min(ans,(ll)((b+a-1)/a)*a-b);
    cout<<min(ans,(ll)abs(b-a))<<nline;
