...
/*
    Write a walk function which finds points at 2 manhatan distance.
    Run a dfs from that point , if you can reach S then YES
    if no possible permutation exists then NO.
*/
int n , m ;
...
bool dfs(int x , int y , vector<string> &grid){
  
    bool flag = false;
    if(valid(x,y) && grid[x][y] != '#'){
        debug("indfs")
        debug(x) debug(y) debug(grid)
        ...
        for (int dx = -1; dx < 2; dx++)
        {
            ...
            {
                if(abs(dx)+abs(dy) == 1 && valid(x+dx,y+dy) && grid[x+dx][y+dy]!='#'){
                    flag = (flag|dfs(x+dx,y+dy,grid));
                    if(flag) return flag;
                }
            }
...
void walk(int xi , int yi , int steps , vector<string> grid ){
    debug(xi) debug(yi) debug(steps)
    debug(grid)
    if(steps>2) return;
    if(steps == 0){
        ...
        for (int dx = -1; dx < 2; dx++)
        {
            for (int dy = -1; dy < 2; dy++)
            {
                if(abs(dx)+abs(dy) == 1 && valid(xi+dx,yi+dy)){
                    debug(grid)
                    walk(xi+dx,yi+dy,steps,grid);
                }
            }
    ...
    if(steps == 2 && valid(xi,yi) && grid[xi][yi]!='#'){
        grid[xi][yi]='#';
        steps++;
        for (int dx = -1; dx < 2; dx++)
        {
            for (int dy = -1; dy < 2; dy++)
            {
                if(abs(dx)+abs(dy) == 1 && valid(xi+dx,yi+dy) && grid[(xi+dx)][yi+dy] != '#'){
                    debug(nline)
                    ans = (ans | dfs(xi+dx,yi+dy,grid) );
                    if(ans) return;
                }
            }
