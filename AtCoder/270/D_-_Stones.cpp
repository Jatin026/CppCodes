...
vi dp(1e4+10,-1);
pair<ll,ll> rec(int n ,int k , vi &v , int f ){
    // if(n==0) return {0,0};
    // ll ans=-1e10;
    // if(f){
    //     for (int i = 0; i < k; i++)
    //     {
    //         if(n-v[k]>=0){
    //             ans=max(ans,v[k]+rec(n-v[k],k,v,!f).first);
    //         }
    //     }
    //     return {ans,0};
    // }
    ...
    //     {
    //         if(n-v[k]>=0){
    //             ans=max(ans,v[k]+rec(n-v[k],k,v,!f).second);
    //         }
    //     }
    //     return {0,ans};
    // }
...
void solve(){
    int n,k;cin>>n>>k;
    vi v(k); for(auto &x : v) cin>>x;
    cout<<rec(n,k,v,1).first;
}
int main(){
    FAST
    int t=1;
