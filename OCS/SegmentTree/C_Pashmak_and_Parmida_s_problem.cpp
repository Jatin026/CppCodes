...
struct segtree{
    ...
    int sum(int l , int r, int x , int lx , int rx,int val){
        vi y;
        if(l>=rx || lx>=r) return (0);
        if(l<=lx && r>=rx) {
            debug(ele[x]) debug(val)
            return (ele[x].end()-lb(all(ele[x]),val+1));
        }
         
            int m = (lx+rx)/2;
            // pair<int,vi> s1=sum(l,r,2*x+1,lx,m,val);
            // pair<int,vi> s2=sum(l,r,2*x+2,m,rx,val);
            // int val = (s1.first+s2.first);
            // y.insert(y.end(),all(s1.second));
            // y.insert(y.end(),all(s2.second));
            // sort(all(y));
