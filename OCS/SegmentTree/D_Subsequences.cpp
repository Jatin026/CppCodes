...
struct segtree{
    ...
    void set(int i , vll v , int x , int lx, int rx){
        if(rx-lx==1){
            sums[x]=v;
            return;
        }
        ...
        for (int j = 0; j < 12; j++)
        {
            sums[x][j]=sums[2*x+1][j]+sums[2*x+2][j];
        }
    }
    void set(int i , vll v){
        set(i,v,0,0,size);
    }
    vll sum(int l , int r, int x , int lx , int rx){
        vll y(13,0);
        if(l>=rx || lx>=r) return y;
        if(l<=lx && r>=rx) return sums[x];
         
            int m = (lx+rx)/2;
            vll s1=sum(l,r,2*x+1,lx,m);
            vll s2=sum(l,r,2*x+2,m,rx);
            vll s3(13);
            for (int i = 0; i < 12; i++)
            {
                s3[i]=s1[i]+s2[i];
