...
// }
// /*
//     trying a brute recursion 

//     m2
//     init + x
//     4 2 or simply 2 ??
//     init + x => 2*(init+x) ,, init + 2*x
// */
...
//             if(steps <= n ) ans = min (ans , ((n-steps)/rem)*2 + (n-steps)%rem + cost);
...
*/
map<pr,ll> dp;
ll rec(ll n, ll target , ll prev){
    debug(n) debug(target) debug(prev)
    if(n == target) return 0;
    if(n>target || prev > target) return 1e9;
    if(dp.count(make_pair(n,prev))) return dp[(make_pair(n,prev))];
    ll ans = 1 + rec(n+1,target,prev);
    if(n!=0 && n!=prev) ans =min(ans,4+rec(n,target,n));
    if(prev!=0 ) ans = min(ans , 2 + rec(n+prev,target,prev));
 
    return dp[(make_pair(n,prev))]=ans;
}
void solve(){
    int n;
    cin >> n;
    Case();
    cout << rec(0,n,0)<<nline;
    dp.clear();
}
int main(){
    FAST
